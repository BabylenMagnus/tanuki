    ...
    pub fn poll(&self) -> Result<Option<Packet>> {
        ...
        if let Some(packet) = packet {
            ...
            match packet.packet_id {
                ...
                PacketId::Open => {
                    // Upstream assumes a second `Open` packet can never arrive
                    // after the initial handshake and treats it as
                    // unreachable. In practice a server can and does resend
                    // one (observed live against `tanuki_api`'s Socket.IO
                    // server on a `TerminalAnsi`-encoding cloud-relay
                    // attach), which turned this into a real panic that
                    // killed the client's poll thread. Ignore it instead of
                    // panicking -- the connection is already open, so there
                    // is nothing this packet needs to trigger.
                }
                PacketId::Upgrade => {
