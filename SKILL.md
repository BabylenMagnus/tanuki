---
name: tanuki
description: "Control Tanuki, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Tanuki or asks to use Tanuki to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires TANUKI_ENV=1."
---

# Tanuki

Tanuki organizes terminals into workspaces, tabs, and panes, recognizes coding agents running inside panes, and exposes the current session through the `tanuki` CLI.

Before issuing any control command, verify that this agent is running inside a Tanuki-managed pane:

```bash
test "${TANUKI_ENV:-}" = 1
```

If the check fails, say that you are not running inside Tanuki and stop. Do not inspect or control the focused Tanuki session from outside Tanuki.

When the check passes, the `tanuki` binary in `PATH` talks to the current session. Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
Do not run bare `tanuki` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `tanuki workspace create` are valid with defaults and will execute.
...
A pane exists whether or not it contains an agent. `agent start` requires an existing available shell pane and never creates, splits, or moves layout. Use pane commands for ordinary processes. Use agent commands when Tanuki must validate agent identity or interpret `idle`, `working`, `blocked`, `done`, and `unknown` lifecycle states.
...
`idle` means the agent is ready for input and its tab has been seen in the focused Tanuki UI. `done` is the same underlying idle state after unseen background work finishes. Focusing the tab or targeting the pane or agent with a focus command marks it seen. CLI reads do not mark it seen. `blocked` means Tanuki recognized an approval or question UI. `unknown` means an agent is present but Tanuki cannot classify it confidently; it does not prove completion.
...
Tanuki injects the caller's context into each managed pane:

```bash
printf '%s\n' "$TANUKI_WORKSPACE_ID" "$TANUKI_TAB_ID" "$TANUKI_PANE_ID"
```
...
```bash
tanuki workspace list
tanuki tab list --workspace "$TANUKI_WORKSPACE_ID"
tanuki pane current --current
tanuki pane list --workspace "$TANUKI_WORKSPACE_ID"
tanuki agent list
```
...
```bash
tanuki pane layout --pane "$TANUKI_PANE_ID"
```
...
```bash
tanuki pane split --current --direction right --cwd "$PWD" --no-focus
```
...
```bash
tanuki agent start reviewer --kind codex --pane <returned-pane-id>
```

Use the kind requested by the user. Run `tanuki agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
tanuki agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

`agent start` returns only after Tanuki detects the expected agent in the same pane and considers it ready for interactive input. It defaults to a 30-second startup timeout.

Submit work through the agent surface:

```bash
tanuki agent prompt reviewer "Review the current diff and report only actionable findings." --wait --timeout 120000
```
...
```bash
tanuki agent send-keys reviewer esc
tanuki agent send-keys reviewer ctrl+c
```

Tanuki validates all keys before writing any bytes. Read the result through the resolved agent:

```bash
tanuki agent get reviewer
tanuki agent read reviewer --source recent-unwrapped --lines 120
```
...
```bash
tanuki pane run <returned-pane-id> "just test"
tanuki pane wait-output <returned-pane-id> --match "test result" --timeout 120000
tanuki pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
`--lines` asks Tanuki for more rows from the pane's available screen and host scrollback. If increasing it does not reveal more of a completed response, the pane is probably running the agent on the terminal's alternate screen. Rows that leave the alternate screen do not enter Tanuki's host scrollback, so a larger line count cannot recover them.
...
- Never run `tanuki server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main Tanuki process. Use named test sessions for experiments that need an isolated server.
