...
just windows-test        # unit tests on Windows (correctly scoped, see below)
just windows-test-one <filter>  # single test filter on Windows
```

On Windows, always scope test/build commands to `--bin tanuki`. The
`tests/*.rs` integration binaries (auto_detect, cli, client_mode, cross_area,
detach_reattach, live_handoff, multi_client, server_headless, api_ping) are
Unix-only (libc `pid_t`/`kill`/`waitpid`, Unix domain sockets,
`std::os::unix`) and fail to compile on Windows. Bare `cargo test`,
`cargo build --tests`, or `cargo nextest run` without a `--bin`/`-E` filter
will try to compile them and error out with `cannot find unix in os` /
`cannot find function kill in crate libc`. Use `just windows-test` (or
`cargo test --bin tanuki ...` directly) instead.

Also on Windows: any test that builds an `App` via `App::new(...)` and then
triggers a real (non-fake) workspace/terminal spawn must set
`app.state.default_shell` explicitly (e.g. via
`crate::app::api::test_support::exiting_test_command()`). It defaults to an
empty string, and `pane_shell_from()` falls back to spawning a real,
interactive `powershell.exe` on Windows when it's empty — this hangs the
test binary forever since tests run single-threaded
(`--test-threads=1`). Tests that only use fake runtimes
(`ensure_test_terminals()`, `TerminalRuntime::test_with_screen_bytes()`) are
unaffected.
