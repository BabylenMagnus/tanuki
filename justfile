...
# Run the unit-test suite on Windows. MUST use `--bin tanuki`, never bare
# `cargo test`/`cargo build --tests` or `cargo nextest run` without a
# `--bin`/`-E` scope: the `tests/*.rs` integration binaries (auto_detect,
# cli, client_mode, cross_area, detach_reattach, live_handoff, multi_client,
# server_headless, api_ping) are Unix-only (libc pid_t/kill/waitpid, Unix
# sockets, std::os::unix) and fail to compile on Windows if pulled in.
windows-test:
    cargo test --bin tanuki --no-fail-fast -- --test-threads=1

# Run one test filter on Windows in isolation (same --bin scoping as windows-test)
windows-test-one filter:
    cargo test --bin tanuki "{{filter}}" -- --exact --nocapture
...
windows-lint:
    ...
    LIBGHOSTTY_VT_SIMD=false cargo clippy --bin tanuki --locked --target x86_64-pc-windows-msvc -- -D warnings
