...
use support::{
    cleanup_test_base, register_runtime_dir, register_spawned_tanuki_pid,
    unregister_spawned_tanuki_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/tanuki-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}

struct SpawnedTanuki {
    _master: Box<dyn MasterPty + Send>,
    child: Box<dyn Child + Send + Sync>,
}

impl Drop for SpawnedTanuki {
    fn drop(&mut self) {
        ...
        if let Some(pid) = pid {
            ...
            unregister_spawned_tanuki_pid(Some(pid));
        }
    }
}

fn cleanup_spawned_tanuki(spawned: SpawnedTanuki, base: PathBuf) {
    drop(spawned);
...
fn spawn_server(
    ...
    fs::create_dir_all(config_home.join("tanuki")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    register_runtime_dir(runtime_dir);
    fs::write(
        config_home.join("tanuki/config.toml"),
        "onboarding = false\n",
    )
    .unwrap();
    ...
    let mut cmd = CommandBuilder::new(env!("CARGO_BIN_EXE_tanuki"));
    cmd.arg("server");
    cmd.env("XDG_CONFIG_HOME", config_home);
    cmd.env("XDG_RUNTIME_DIR", runtime_dir);
    cmd.env("TANUKI_SOCKET_PATH", api_socket_path);
    cmd.env_remove("TANUKI_CLIENT_SOCKET_PATH");
    cmd.env("SHELL", "/bin/sh");
    cmd.env_remove("TANUKI_ENV");

    let child = pair.slave.spawn_command(cmd).unwrap();
    register_spawned_tanuki_pid(child.process_id());
    drop(pair.slave);
...
/// Spawn `tanuki` (no subcommand) — the auto-detect launch path.
fn spawn_tanuki_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
    _client_socket_path: &Path,
...
/// Spawn `tanuki --no-session` — the monolithic escape hatch.
fn spawn_tanuki_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedTanuki {
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_tanuki"));
    command.args(args);
    command.env("TANUKI_SOCKET_PATH", socket_path);
...
/// Running `tanuki` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    let api_socket = runtime_dir.join("tanuki.sock");
    let client_socket = runtime_dir.join("tanuki-client.sock");
    ...
    // Run `tanuki` (no subcommand) — should auto-detect, spawn server, attach as client.
    let tanuki = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = tanuki.child.process_id().expect("client should have PID");
    assert!(
        process_exists(client_pid),
        "client process should be running"
    );

    cleanup_spawned_tanuki(tanuki, base);
}

/// Running `tanuki` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `tanuki` (no subcommand) — should detect the running server and attach.
    let client = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_tanuki(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_tanuki(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `TANUKI_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `tanuki` with custom socket paths.
    let tanuki = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `tanuki --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `tanuki --no-session` — monolithic mode, no server/client.
    let tanuki = spawn_tanuki_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one tanuki process.
    let client_pid = tanuki.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `tanuki workspace list` through the server's API socket.
    ...
    // Test `tanuki pane list` through the server's API socket.
...
/// persists after the client exits, and a new `tanuki` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `tanuki` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `tanuki` again — should detect the running server and reattach.
    let client2 = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_tanuki(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set TANUKI_SOCKET_PATH or TANUKI_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("tanuki.sock");
    let client_socket = config_home.join(app_dir_name).join("tanuki-client.sock");
    ...
    cmd.env_remove("TANUKI_SOCKET_PATH");
    cmd.env_remove("TANUKI_CLIENT_SOCKET_PATH");

    let child = pair.slave.spawn_command(cmd).unwrap();
    register_spawned_tanuki_pid(child.process_id());
    drop(pair.slave);
    let server = SpawnedTanuki {
        _master: pair.master,
        child,
    };
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_tanuki_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("tanuki-client.log");
    let server_log = log_dir.join("tanuki-server.log");
    let monolith_log = log_dir.join("tanuki.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in tanuki.log: {monolith_content}"
    );

    cleanup_spawned_tanuki(spawned, base);
}
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_tanuki_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_tanuki"))
        .env("XDG_CONFIG_HOME", &config_home)
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("TANUKI_SOCKET_PATH", &api_socket)
        .env_remove("TANUKI_CLIENT_SOCKET_PATH")
        .env("TANUKI_ENV", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("nested tanuki is disabled by default"),
