...
fn spawn_tanuki(config_home: &Path, runtime_dir: &Path, socket_path: &Path) -> SpawnedTanuki {
    spawn_tanuki_with_options(config_home, runtime_dir, socket_path, None, "/bin/sh")
}

fn spawn_tanuki_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
) -> SpawnedTanuki {
    spawn_tanuki_with_options(
        config_home,
        runtime_dir,
        socket_path,
        path_override,
...
#[cfg(target_os = "linux")]
fn spawn_tanuki_with_shell(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    shell: &str,
) -> SpawnedTanuki {
    spawn_tanuki_with_options(config_home, runtime_dir, socket_path, None, shell)
}

fn spawn_tanuki_with_options(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
    ...
    cmd.env("TANUKI_SOCKET_PATH", socket_path);
...
fn ping_over_socket_returns_version() {
    ...
    let socket_path = runtime_dir.join("tanuki.sock");

    let child = spawn_tanuki(&config_home, &runtime_dir, &socket_path);
    ...
    cleanup_spawned_tanuki(child, base);
}
...
fn server_reload_agent_manifests_reports_runtime_override() {
    ...
    let override_dir = config_home.join("tanuki-dev").join("agent-detection");
...
fn pane_info_reports_foreground_cwd_without_changing_pane_cwd() {
    ...
    let child = spawn_tanuki_with_shell(&config_home, &runtime_dir, &socket_path, "/bin/bash");
    wait_for_socket(&socket_path, Duration::from_secs(5));
...
fn agent_start_targets_existing_pane_over_socket() {
    ...
    fs::write(&fake_pi, "#!/bin/sh\nTANUKI_AGENT=pi exec /bin/sleep 20\n").unwrap();
    fs::set_permissions(&fake_pi, fs::Permissions::from_mode(0o755)).unwrap();

    let child = spawn_tanuki_with_path(&config_home, &runtime_dir, &socket_path, Some(&bin));
    wait_for_socket(&socket_path, Duration::from_secs(5));
...
fn events_subscribe_streams_workspace_tab_and_agent_events() {
    ...
    let child = spawn_tanuki_with_path(
        &config_home,
        &runtime_dir,
        &socket_path,
...
fn pane_report_agent_updates_effective_state() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_5","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"working","message":"thinking","agent_session_path":"{}"}}}}"#,
            pane_id,
    ...
    let metadata = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_metadata","method":"pane.report_metadata","params":{{"pane_id":"{}","source":"user:pi-display","agent":"pi","applies_to_source":"tanuki:pi","title":"Refactor auth","display_agent":"Pi auth","state_labels":{{"working":"deep in the mines"}},"tokens":{{"summary":"reviewing auth","model":"opus"}}}}}}"#,
            pane_id
        ),
    );
    assert_eq!(metadata["result"]["type"], "ok");
...
fn pane_release_agent_suppresses_reacquire_during_graceful_exit() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"working"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(hook["result"]["type"], "ok");
    ...
    let released = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_5","method":"pane.release_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi"}}}}"#,
            pane_id
        ),
    );
...
fn pane_clear_agent_authority_restores_fallback_state() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(hook["result"]["type"], "ok");
    ...
    let cleared = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_5","method":"pane.clear_agent_authority","params":{{"pane_id":"{}","source":"tanuki:pi"}}}}"#,
            pane_id
        ),
    );
...
fn metadata_status_subscription_filter_and_ttl_expiry_are_observable() {
    ...
    let report_agent = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_meta_sub_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"working"}}}}"#,
            pane_id
        ),
    );
    ...
    let metadata = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_meta_sub_3","method":"pane.report_metadata","params":{{"pane_id":"{}","source":"user:pi-display","agent":"pi","applies_to_source":"tanuki:pi","title":"filtered out"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(metadata["result"]["type"], "ok");
    ...
    let metadata = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_meta_sub_4","method":"pane.report_metadata","params":{{"pane_id":"{}","source":"user:pi-display","agent":"pi","applies_to_source":"tanuki:pi","title":"short lived","ttl_ms":100}}}}"#,
            pane_id
        ),
    );
    assert_eq!(metadata["result"]["type"], "ok");
