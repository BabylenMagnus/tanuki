...
fn run_claude_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/claude/tanuki-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_codex_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/codex/tanuki-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_copilot_hook(hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/copilot/tanuki-agent-state.sh",
        &[],
        hook_input,
    )
}
...
fn run_devin_hook(
    ...
    run_shell_hook_with_env(
        "src/integration/assets/devin/tanuki-agent-state.sh",
        &[action],
        hook_input,
        envs,
    )
...
fn run_shell_hook_with_env(
    ...
    command
        ...
        .args(args)
        .env("TANUKI_ENV", "1")
        .env("TANUKI_SOCKET_PATH", &socket_path)
        .env("TANUKI_PANE_ID", "p_test")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
fn devin_hook_reports_session_id_from_stdin_without_state() {
    let request = run_devin_hook(
        ...
        &[("TANUKI_DEVIN_LIST_JSON", r#"[{"id":"older-session"}]"#)],
    )
