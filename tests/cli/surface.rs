...
fn pane_run_sends_one_send_input_request_with_enter_key() {
    ...
    let socket_path = base.join("tanuki.sock");
...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "tanuki {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
...
fn subcommand_help_explains_automation_semantics_without_a_server() {
    ...
    for (args, expected) in cases {
        ...
        assert!(
            ...
            "tanuki {} help did not contain {expected:?}: {stdout}",
            args.join(" ")
        );
    }
}
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_tanuki"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_tanuki"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("tanuki wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_tanuki"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("tanuki agent send-keys"));
    assert!(!stderr.contains("tanuki agent send <"));
}
...
fn agent_cli_rejects_invalid_wait_and_rename_grammar_locally() {
    for args in [
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_tanuki"))
            .args(args)
            .env("TANUKI_SOCKET_PATH", "/nonexistent/tanuki.sock")
            .output()
            .unwrap();
        assert_eq!(
            output.status.code(),
            Some(2),
            "tanuki {}: stdout={} stderr={}",
            args.join(" "),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef tanuki"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("tanuki api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Tanuki API schema"), "stdout: {stdout}");
    assert!(
        stdout.contains("Use `tanuki api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
...
fn api_schema_output_writes_bundled_schema_to_file() {
    let base = unique_test_dir();
    fs::create_dir_all(&base).unwrap();
    let schema_path = base.join("tanuki-api.schema.json");
