...
fn agent_wait_exits_immediately_when_status_already_matches() {
    ...
    let reported = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_cli_immediate_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
...
fn agent_wait_times_out_when_status_does_not_match() {
    ...
    let reported = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_cli_timeout_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"tanuki:pi","agent":"pi","state":"working"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(reported["result"]["type"], "ok");
...
fn agent_wait_exits_when_done_status_matches() {
    ...
    let tanuki = spawn_tanuki_with_path(
        &config_home,
        &runtime_dir,
        &socket_path,
