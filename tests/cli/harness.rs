...
pub(super) use crate::support::{
    ...
    unregister_spawned_tanuki_pid, CURRENT_PROTOCOL,
};
...
pub(super) fn create_committed_repo(path: &Path) {
    ...
    run_git(path, &["config", "user.email", "tanuki@example.invalid"]);
    run_git(path, &["config", "user.name", "Tanuki Test"]);
    fs::write(path.join("README.md"), "test\n").unwrap();
    run_git(path, &["add", "README.md"]);
    run_git(path, &["commit", "--quiet", "-m", "initial"]);
}

pub(super) struct SpawnedTanuki {
...
pub(super) fn cleanup_spawned_tanuki(spawned: SpawnedTanuki, base: PathBuf) {
    drop(spawned);
...
pub(super) fn spawn_tanuki(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedTanuki {
    spawn_tanuki_with_config(
        config_home,
        runtime_dir,
        socket_path,
        None,
...
pub(super) fn spawn_tanuki_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedTanuki {
...
pub(super) fn spawn_named_server(
    ...
    command
        ...
        .env_remove("TANUKI_SOCKET_PATH")
        .env_remove("TANUKI_CLIENT_SOCKET_PATH")
        .env_remove("TANUKI_ENV")
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());

    let child = command.spawn().unwrap();
    register_spawned_tanuki_pid(Some(child.id()));
...
pub(super) fn run_named_cli_with_env_and_socket_override(
    ...
    if let Some(socket_override) = socket_override {
        command.env("TANUKI_SOCKET_PATH", socket_override);
    } else {
        command.env_remove("TANUKI_SOCKET_PATH");
    }
...
pub(super) fn spawn_tanuki_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
...
pub(super) fn spawn_tanuki_with_config(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
...
pub(super) fn parse_cli_json_output(
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `tanuki {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
