...
use support::{
    ...
    register_runtime_dir, register_spawned_tanuki_pid, unregister_spawned_tanuki_pid,
...
fn spawn_client_process(
    ...
    cmd.arg("client");
    cmd.env("TANUKI_DISABLE_SOUND", "1");
        ...
        match read_server_message(&mut stream) {
            Ok((1, payload)) => {
                ...
                if last_frame_text.contains("config.toml")
                    && last_frame_text.contains("tanuki config check")
                {
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_tanuki"))
        .arg("client")
        .env("TANUKI_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is tanuki server running?"),
        ...
        while Instant::now() < deadline {
            match thin_reader.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if output.to_lowercase().contains("tanuki:") {
                        break;
                    }
                }
