//! Windows clipboard-image decoding.
//!
//! Converts whatever image payload the Windows clipboard is holding (a
//! registered "PNG" format, or a legacy `CF_DIB`/`CF_DIBV5` device-independent
//! bitmap) into validated PNG bytes bounded by
//! [`crate::protocol::MAX_CLIPBOARD_IMAGE_PAYLOAD`], for bridging into
//! [`super::ClipboardImage`].
...
/// Validates that `bytes` is a well-formed, size-bounded PNG and returns just
/// the logical PNG payload (some clipboard producers append trailing
/// allocator padding after the `IEND` chunk).
...
/// Converts a `CF_DIB`/`CF_DIBV5` device-independent bitmap to PNG bytes.
