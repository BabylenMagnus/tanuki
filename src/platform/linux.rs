...
const PROCESS_DETECTION_ENV_VAR: &str = "TANUKI_PROCESS_DETECTION";
...
/// Read a Tanuki agent identity hint from a process environment.
...
mod tests {
    ...
    fn read_clipboard_image_rejects_xclip_text_served_for_image_target() {
        let _guard = env_lock().lock().unwrap();
        let temp_dir =
            std::env::temp_dir().join(format!("tanuki-fake-xclip-{}", std::process::id()));
    ...
    fn read_clipboard_image_rejects_wayland_xclip_fallback_text_for_image_target() {
        let _guard = env_lock().lock().unwrap();
        let temp_dir =
            std::env::temp_dir().join(format!("tanuki-fake-wayland-xclip-{}", std::process::id()));
        std::fs::create_dir_all(&temp_dir).expect("temp dir should be created");
    ...
    fn desktop_notification_separates_option_like_titles() {
        ...
        let path =
            std::env::temp_dir().join(format!("tanuki-notify-send-args-{}", std::process::id()));
        let script = "printf '%s\\n' \"$@\" > \"$TANUKI_NOTIFY_ARGS\"";
    ...
    fn scrollback_editor_argv_preserves_unix_editor_shell_semantics() {
        let path = std::path::Path::new("/tmp/tanuki scrollback.txt");
        let argv = scrollback_editor_argv(path).unwrap();

        assert_eq!(argv[0], "/bin/sh");
        assert_eq!(argv[1], "-c");
        assert!(argv[2].contains("EDITOR:-vi"));
        assert!(argv[2].contains("/tmp/tanuki scrollback.txt"));
    }
}
