...
fn osc_debug_enabled_from_env() -> bool {
    std::env::var("TANUKI_DEBUG_OSC_EVIDENCE")
        .map(|value| {
            matches!(
...
pub(super) fn maybe_filter_primary_screen_scrollback_clear<'a>(
    ...
    // scrollback inside tanuki. Keep the hack scoped to Droid on the primary
...
mod tests {
    ...
    fn reported_cwd_parses_file_uri_and_bare_paths() {
        assert_eq!(
            parse_reported_cwd(b"file:///tmp/tanuki%20repo"),
            Some(std::path::PathBuf::from("/tmp/tanuki repo"))
        );
        assert_eq!(
            parse_reported_cwd(b"C:\\Users\\tanuki\\src\\tanuki"),
            Some(std::path::PathBuf::from("C:\\Users\\tanuki\\src\\tanuki"))
        );
        assert_eq!(
