...
pub(super) fn command() -> Command {
    let command = Command::new("tanuki")
        ...
        .arg(option("remote", "TARGET").help("Attach through SSH to a remote Tanuki server"))
        .arg(
...
fn write_requested_help(args: &[String], output: &mut impl Write) -> std::io::Result<bool> {
    ...
    let mut path = vec!["tanuki".to_string()];
...
fn notification_command() -> Command {
    Command::new("notification")
        .about("Show Tanuki notifications")
        .subcommand(
            Command::new("show")
    ...
    Command::new("agent")
        ...
        .subcommand(
            Command::new("read")
                ...
                .override_usage("tanuki agent read <TARGET> [OPTIONS]")
        ...
        .subcommand(
            Command::new("prompt")
                ...
                .override_usage("tanuki agent prompt <TARGET> <TEXT> [OPTIONS]")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("rename")
                ...
                .override_usage("tanuki agent rename <TARGET> <NAME>|--clear")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("wait")
                ...
                .override_usage("tanuki agent wait <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(
                    option("until", "STATUS")
                        .action(ArgAction::Append)
        ...
        .subcommand(
            Command::new("attach")
                ...
                .override_usage("tanuki agent attach <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(flag("takeover")),
        )
        .subcommand(
            Command::new("start")
                ...
                .override_usage(
                    "tanuki agent start <NAME> --kind <KIND> --pane <ID> [OPTIONS] [-- [AGENT_ARG]...]",
                )
    ...
    Command::new("pane")
        ...
        .subcommand(
            Command::new("send-text")
                ...
                .arg(required("text", "TEXT"))
                .after_help(
                    "next: tanuki pane run <PANE_ID> <COMMAND> sends text and Enter in one call",
                ),
        )
        .subcommand(
            Command::new("send-keys")
...
mod tests {
    ...
    fn every_spec_subcommand_renders_short_and_long_help() {
        ...
        for path in paths {
            for flag in ["-h", "--help"] {
                let mut args = vec!["tanuki".to_string()];
                args.extend(path.iter().cloned());
                args.push(flag.to_string());
                let mut output = Vec::new();
                assert!(
                    super::write_requested_help(&args, &mut output).unwrap(),
                    "help was not handled for tanuki {} {flag}",
                    path.join(" ")
                );
                let output = String::from_utf8(output).unwrap();
                assert!(
                    output.contains(&format!("Usage: tanuki {}", path.join(" "))),
                    "unexpected help for tanuki {}: {output}",
                    path.join(" ")
                );
            }
        }
        ...
        for (path, options) in [
            ...
            for option in options {
                assert!(
                    ...
                    "tanuki {} --{option} should be required",
                    path.join(" ")
                );
            }
        }
    ...
    fn agent_prompt_until_requires_wait() {
        let error = super::command()
            .try_get_matches_from([
                "tanuki", "agent", "prompt", "reviewer", "hello", "--until", "idle",
            ])
            .unwrap_err();
        assert_eq!(
            error.kind(),
    ...
    fn agent_rename_requires_exactly_one_name_or_clear() {
        for valid in [
            &["tanuki", "agent", "rename", "reviewer", "worker"][..],
            &["tanuki", "agent", "rename", "reviewer", "--clear"][..],
        ] {
            assert!(super::command().try_get_matches_from(valid).is_ok());
        }
        for invalid in [
            &["tanuki", "agent", "rename", "reviewer"][..],
            &["tanuki", "agent", "rename", "reviewer", "worker", "--clear"][..],
        ] {
        ...
        assert!(String::from_utf8(help)
            .unwrap()
            .contains("Usage: tanuki agent rename <TARGET> <NAME>|--clear"));
    }
    ...
    fn long_help(path: &[&str]) -> String {
        ...
        assert!(
            super::write_requested_help(&args, &mut output).unwrap(),
            "help was not handled for tanuki {}",
            path.join(" ")
        );
    ...
    fn next_step_hints_render_without_replacing_existing_after_help() {
        ...
        assert!(
            agent_start.contains("next: tanuki agent prompt <TARGET> <TEXT> --wait"),
    ...
    fn completion_generation_succeeds_for_every_supported_shell() {
        for shell in [
            ...
            let mut output = Vec::new();
            clap_complete::generate(shell, &mut cmd, "tanuki", &mut output);
