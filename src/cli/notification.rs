...
use crate::config::ToastTanukiPosition;
...
fn notification_show(args: &[String]) -> std::io::Result<i32> {
    let params = match parse_notification_show_args(args) {
        ...
        Err(NotificationShowArgError::Usage) => {
            eprintln!(
                "usage: tanuki notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
            );
            return Ok(2);
        }
...
fn parse_toast_position(value: &str) -> Result<ToastTanukiPosition, NotificationShowArgError> {
    match value {
        "top-left" => Ok(ToastTanukiPosition::TopLeft),
        "top-right" => Ok(ToastTanukiPosition::TopRight),
        "bottom-left" => Ok(ToastTanukiPosition::BottomLeft),
        "bottom-right" => Ok(ToastTanukiPosition::BottomRight),
...
fn print_notification_help() {
    eprintln!("tanuki notification commands:");
    eprintln!(
        "  tanuki notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
    );
}
    ...
    fn notification_show_args_parse_title_body_and_position() {
        ...
        assert_eq!(
            ...
            NotificationShowParams {
                ...
                position: Some(ToastTanukiPosition::TopRight),
