const API_SCHEMA_JSON: &str = include_str!("../../docs/next/api/tanuki-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: tanuki api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Tanuki API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `tanuki api schema --json` to print the full schema.\nUse `tanuki api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("tanuki api commands:");
    eprintln!("  tanuki api snapshot");
    eprintln!("  tanuki api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: tanuki api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Tanuki API schema"));
        assert!(text.contains("Use `tanuki api schema --json`"));
        assert!(text.len() < 400);
    }
}
