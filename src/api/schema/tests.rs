...
fn generated_protocol_schema_artifact_is_current() {
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("docs/next/api/tanuki-api.schema.json");

    if std::env::var_os("TANUKI_UPDATE_API_SCHEMA").is_some() {
    ...
    let expected = std::fs::read_to_string(&path).unwrap_or_else(|err| {
        panic!(
            "failed to read {}; run `TANUKI_UPDATE_API_SCHEMA=1 just test-one generated_protocol_schema_artifact_is_current`: {err}",
            path.display()
        )
    });
    assert_eq!(
        expected,
        actual,
        "generated API schema artifact is stale; run `TANUKI_UPDATE_API_SCHEMA=1 just test-one generated_protocol_schema_artifact_is_current`"
    );
}
...
fn client_window_title_requests_round_trip() {
    let set = Request {
        ...
        method: Method::ClientWindowTitleSet(ClientWindowTitleSetParams {
            title: "tanuki api".into(),
        }),
    };
    let json = serde_json::to_value(&set).unwrap();
    assert_eq!(json["method"], "client.window_title.set");
    assert_eq!(json["params"]["title"], "tanuki api");
    let restored: Request = serde_json::from_value(json).unwrap();
        ...
        result: ResponseResult::WorktreeCreated {
            workspace: WorkspaceInfo {
                ...
                worktree: Some(WorkspaceWorktreeInfo {
                    repo_key: "/repo/tanuki/.git".into(),
                    repo_name: "tanuki".into(),
                    repo_root: "/repo/tanuki".into(),
                    checkout_path: "/worktrees/tanuki/worktree-api".into(),
            ...
            root_pane: PaneInfo {
                ...
                tab_id: "w_1:1".into(),
                focused: true,
                cwd: Some("/worktrees/tanuki/worktree-api".into()),
                foreground_cwd: None,
                label: None,
                agent: None,
                title: None,
            ...
            worktree: WorktreeInfo {
                path: "/worktrees/tanuki/worktree-api".into(),
                branch: Some("worktree/api".into()),
                is_bare: false,
                is_detached: false,
                is_prunable: false,
...
fn plugin_link_list_unlink_round_trip() {
    ...
    let plugin = InstalledPluginInfo {
        ...
        version: "0.1.0".into(),
        min_tanuki_version: crate::build_info::BASE_VERSION.into(),
        description: Some("Prepare new worktrees".into()),
        manifest_path: "/plugins/worktree-bootstrap/tanuki-plugin.toml".into(),
    ...
    let root = LayoutNode::Split {
        ...
        second: Box::new(LayoutNode::Pane {
            pane: LayoutPane {
                ...
                env: HashMap::from([("TANUKI_ROLE".into(), "tests".into())]),
                ..Default::default()
            },
        }),
    };
...
fn plugin_pane_open_request_round_trips() {
    let request = Request {
        ...
        method: Method::PluginPaneOpen(PluginPaneOpenParams {
            ...
            cwd: Some("/tmp".into()),
            focus: true,
            env: [("TANUKI_ROLE".to_string(), "board".to_string())].into(),
        }),
    };
    ...
    assert_eq!(json["params"]["env"]["TANUKI_ROLE"], "board");
    let restored: Request = serde_json::from_value(json).unwrap();
    assert_eq!(restored, request);
}
