...
fn prepare_socket_path(path: &Path) -> std::io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "tanuki is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn unique_test_path(name: &str) -> PathBuf {
        ...
        // On Unix, sockaddr_un's sun_path is capped (~104-108 bytes); macOS's
        // std::env::temp_dir() is often a long per-process /private/var/folders/...
        // path, so a name+pid+nanos suffix there can overflow it. `/tmp` is
        // short and stable on every Unix CI runner. Windows named pipes don't
        // share that length limit, so keep the descriptive temp_dir() path there.
        #[cfg(unix)]
        {
            let _ = name;
        ...
        #[cfg(windows)]
        {
            std::env::temp_dir().join(format!("tanuki-{name}-{filename}"))
        }
    }
    ...
    fn socket_path_prefers_explicit_env_override() {
        ...
        let unique = format!("/tmp/tanuki-test-{}.sock", std::process::id());
