//! Headless server mode — runs the tanuki event loop without a real terminal.
//!
//! The server:
//! - Does not enter raw mode or read stdin
//! - Creates and listens on both `tanuki.sock` (existing JSON API) and
//!   `tanuki-client.sock` (new binary protocol)
...
/// The headless server — runs the tanuki event loop without a real terminal.
        ...
        while !should_quit.load(Ordering::Acquire) {
            ...
            std::thread::spawn(move || {
                if let Err(err) = crate::server::client_transport::handle_client_handshake(
                    crate::ipc::Transport::Local(stream),
                    client_id,
                    &server_event_tx,
                    &should_quit,
                ) {
...
impl HeadlessServer {
    ...
    ///
    /// Only called from the Unix-only live-handoff import path; on other
    /// platforms nothing calls this, hence the `cfg_attr` below.
    ...
    pub fn new(
        ...
        Self::new_with_cloud_host(app, config_diagnostics, api_tx, api_server, false)
    }

    /// Same as [`HeadlessServer::new`], with an explicit `cloud_host` flag.
    ///
    /// When `true`, this additionally starts a
    /// [`crate::remote::cloud::CloudHostTransport`] alongside the local
    /// socket listener, so viewers can attach to this server over the
    /// Tanuki backend relay in addition to (not instead of) local clients.
    /// Defaults to `false` via [`HeadlessServer::new`] -- no CLI flag wires
    /// this up yet; that is deliberately left to a follow-up task (see
    /// `src/remote/cloud.rs`).
    pub fn new_with_cloud_host(
        app: app::App,
        config_diagnostics: &[String],
        api_tx: Option<api::ApiRequestSender>,
        api_server: Option<api::ServerHandle>,
        ...
        let cloud_host_transport = if cloud_host {
            ...
            match crate::remote::cloud::CloudHostTransport::spawn(
                ...
                Err(err) => {
                    warn!(err = %err, "failed to start cloud host relay; continuing local-only");
                    None
                }
            }
        } else {
    ...
    fn perform_live_handoff(
        ...
        if pane_by_terminal.len() > crate::server::handoff::MAX_FDS_PER_HANDOFF {
            ...
            return Err(io::Error::new(
                ...
                format!(
                    "live handoff supports at most {} panes in one update; close panes or restart tanuki normally",
            ...
            ServerEvent::ClientPasteRejected {
                ...
                self.send_to_client(
                    ...
                    ServerMessage::Notify {
                        ...
                        body: Some(format!(
                            "Input message is {size} bytes; Tanuki's limit is {max} bytes"
                        )),
                    },
                );
                false
    ...
    fn handle_api_request_with_shutdown_check_inner(
        ...
        // Tanuki delivery renders the toast in-frame and must not ask clients to
...
pub fn run_server() -> io::Result<()> {
    ...
    let cloud_host = args[2..].iter().any(|arg| arg == "--cloud-host");
    ...
    let _api_server = match api::start_server(api_tx.clone(), event_hub.clone()) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: tanuki server is already running");
    ...
    let result = rt.block_on(async {
        ...
        let mut server = match HeadlessServer::new_with_cloud_host(
            app,
        ...
        if cloud_host && server.cloud_host.is_none() {
            eprintln!(
                "warning: --cloud-host requested but the cloud relay connection failed; continuing local-only (see logs)"
            );
        }
        print_ready_message(
            &api::socket_path(),
            &client_socket_path(),
            server.cloud_host.is_some(),
        );
...
fn run_handoff_import_server(socket_path: &Path, token: &str) -> io::Result<()> {
    ...
    let result = rt.block_on(async {
        ...
        if std::env::var("TANUKI_TEST_HANDOFF_IMPORT_FAIL").as_deref() == Ok("after_restored") {
...
fn init_logging() {
    crate::logging::init_file_logging("tanuki-server.log");
}
        ...
        for i in 0..=crate::app::APP_EVENT_DRAIN_LIMIT {
            server
                ...
                .try_send(AppEvent::UpdateReady {
                    ...
                    install_command: "tanuki install".into(),
                })
                .unwrap();
        }
...
"#,
        ...
        assert!(content.contains("delivery = \"tanuki\""));
    ...
    fn invalid_server_keybindings_apply_valid_subset_after_settings_save_without_caching_local_keybindings(
        ...
        let path = std::env::temp_dir().join(format!(
            "tanuki-headless-invalid-settings-{}-{}.toml",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map(|d| d.as_nanos())
...
"#,
    ...
    fn headless_scheduled_tasks_expire_agent_metadata() {
        ...
        assert!(
            server.handle_internal_event_with_forwarding(AppEvent::HookMetadataReported {
                ...
                applies_to_source: Some("tanuki:pi".into()),
    ...
    async fn headless_scheduled_tasks_do_not_start_pending_agent_resume_when_geometry_dirty() {
        ...
        server
            ...
            dedupe_key: "tanuki:codex\0codex\0Id\0codex-session".into(),
        });
    ...
    fn oversized_paste_rejection_notifies_only_the_sending_client() {
        ...
        match read_server_message(
            ...
            ServerMessage::Notify {
                ...
                assert_eq!(
                    body.as_deref(),
                    Some("Input message is 5000012 bytes; Tanuki's limit is 1048576 bytes")
                );
            }
    ...
    #[test]
    fn tanuki_toast_delivery_keeps_toast_in_frame_without_client_notify() {
        let mut server = test_headless_server();
        let (client_tx, client_control_rx, _client_rx) = test_client_writer();
        ...
        server.foreground_client_id = Some(1);
        server.app.state.toast_config.delivery = crate::config::ToastDelivery::Tanuki;

        let changed = server.handle_internal_event_with_forwarding(AppEvent::UpdateReady {
            version: "9.9.9".to_string(),
            install_command: "tanuki update".into(),
        });
        ...
        assert!(
            client_control_rx
                .recv_timeout(Duration::from_millis(50))
                .is_err(),
            "tanuki delivery should render in-frame instead of forwarding a client-local notification"
        );
    }
    ...
    fn system_toast_delivery_forwards_system_notify_kind() {
        ...
        match read_server_message(
            ...
            ServerMessage::Notify {
                ...
                assert_eq!(
                    body.as_deref(),
                    Some("detach, run `tanuki update`, then follow its restart guidance")
                );
            }
            ...
            request: api::schema::Request {
                ...
                method: api::schema::Method::NotificationShow(
                    api::schema::NotificationShowParams {
                        ...
                        position: Some(crate::config::ToastTanukiPosition::TopLeft),
    ...
    #[test]
    fn notification_show_api_tanuki_toast_expires_headless() {
        let mut server = test_headless_server();
        server.app.state.toast_config.delivery = crate::config::ToastDelivery::Tanuki;
    ...
    #[test]
    fn notification_show_api_forwards_sound_for_tanuki_delivery() {
        let mut server = test_headless_server();
        let (client_tx, client_control_rx, _client_rx) = test_client_writer();
