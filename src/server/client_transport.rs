...
fn write_framed_bytes(stream: &mut Transport, data: &[u8]) -> bool {
...
mod tests {
    ...
    fn local_stream_pair(name: &str) -> (LocalStream, Transport, TestSocketPath) {
        let path = unique_test_path(name);
        let _ = std::fs::remove_file(&path);
        let listener = crate::ipc::bind_local_listener(&path).unwrap();
        let client = crate::ipc::connect_local_stream(&path).unwrap();
        let server = listener.accept().unwrap();
        (client, Transport::Local(server), TestSocketPath(path))
    }
    ...
    fn client_writer_closes_queue_after_socket_write_failure() {
        ...
        #[cfg(not(windows))]
        {
            let Transport::Local(local) = &server_stream else {
                unreachable!("local_stream_pair always returns Transport::Local")
            };
            local
