"""Hermes plugin installed by Tanuki to report agent lifecycle state."""

# TANUKI_INTEGRATION_ID=hermes
# TANUKI_INTEGRATION_VERSION=3
...
_SOURCE = "tanuki:hermes"
_AGENT = "hermes"


def _base_params() -> tuple[str, str] | None:
    if os.environ.get("TANUKI_ENV") != "1":
        return None
    pane_id = os.environ.get("TANUKI_PANE_ID", "").strip()
    socket_path = os.environ.get("TANUKI_SOCKET_PATH", "").strip()
