...
beforeEach(() => {
  ...
  process.env.TANUKI_SOCKET_PATH = "test.sock";
  process.env.TANUKI_PANE_ID = "test:p1";
});

async function loadPlugin() {
  importCounter += 1;
  const { TanukiAgentStatePlugin } = await import(`./tanuki-agent-state.js?test=${importCounter}`);
  return TanukiAgentStatePlugin();
}
