...
// TANUKI_INTEGRATION_ID=opencode
// TANUKI_INTEGRATION_VERSION=9

import net from "node:net";

const SOURCE = "tanuki:opencode";
...
export const TanukiAgentStatePlugin = async () => {
  if (
    process.env.TANUKI_ENV !== "1" ||
    !process.env.TANUKI_SOCKET_PATH ||
    !process.env.TANUKI_PANE_ID
  ) {
    return {};
  }
    ...
    event: async ({ event }) => {
      ...
      switch (type) {
        case "session.created":
          ...
          // creates are dropped above). Signal it so tanuki replaces the pane's
