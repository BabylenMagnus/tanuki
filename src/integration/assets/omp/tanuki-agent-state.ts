...
// TANUKI_INTEGRATION_VERSION=6
// @ts-nocheck

import net from "node:net";

const TANUKI_ENV = process.env.TANUKI_ENV;
const socketPath = process.env.TANUKI_SOCKET_PATH;
const socketEndpoint =
  process.platform === "win32" && socketPath ? `\\\\.\\pipe\\${socketPath}` : socketPath;
const paneId = process.env.TANUKI_PANE_ID;
const source = "tanuki:omp";

function enabled() {
  return TANUKI_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("TANUKI_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("TANUKI_OMP_RETRY_GRACE_MS", 2500);
...
function shouldReleaseOnSessionShutdown(event: any): boolean {
  ...
  // should release Tanuki's full-lifecycle authority.
...
export default function (pi) {
  ...
  pi.events.on("tanuki:blocked", (data) => {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
