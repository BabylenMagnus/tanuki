...
const originalEnvironment = {
  TANUKI_ENV: process.env.TANUKI_ENV,
  TANUKI_OMP_IDLE_DEBOUNCE_MS: process.env.TANUKI_OMP_IDLE_DEBOUNCE_MS,
  TANUKI_PANE_ID: process.env.TANUKI_PANE_ID,
  TANUKI_SOCKET_PATH: process.env.TANUKI_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/tanuki-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/tanuki-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/tanuki-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/tanuki-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  ...
  process.env.TANUKI_PANE_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `tanuki-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `tanuki-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();

    const { TanukiAgentStatePlugin } = await importFresh(socketPlugin.modulePath);
    const plugin = await TanukiAgentStatePlugin();
    await plugin.event({
      event: {
...
test("OpenCode stays disabled without the Tanuki socket environment", async () => {
  process.env.TANUKI_ENV = "1";
  process.env.TANUKI_PANE_ID = "test:p1";
  delete process.env.TANUKI_SOCKET_PATH;

  const { TanukiAgentStatePlugin } = await importFresh("./opencode/tanuki-agent-state.js");

  expect(await TanukiAgentStatePlugin()).toEqual({});
});

for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `tanuki-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
...
test("Pi reports idle only after the agent settles", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/tanuki-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("tanuki:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("tanuki:blocked")?.({ active: false }, context);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `tanuki-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.TANUKI_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/tanuki-agent-state.ts");
  install(pi);
