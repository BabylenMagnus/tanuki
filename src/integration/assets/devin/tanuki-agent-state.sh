...
# TANUKI_INTEGRATION_ID=devin
# TANUKI_INTEGRATION_VERSION=2

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/tanuki-devin-hook.XXXXXX")" || exit 0
...
TANUKI_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("TANUKI_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("TANUKI_HOOK_INPUT_FILE"))
