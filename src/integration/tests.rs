...
fn enforce_agent_version_warns_when_binary_missing() {
    let requirement = AgentVersionRequirement {
        label: "kimi code",
        binary: "tanuki-test-binary-that-does-not-exist",
        args: &["--version"],
        min_version: "0.14.0",
    };
...
fn unique_base() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "tanuki-integration-install-test-{}-{}",
        std::process::id(),
...
fn integration_recommendation_installs_available_or_outdated_targets() {
    let mut recommendation = IntegrationRecommendation {
        ...
        command: "claude",
        available: false,
        path: PathBuf::from("/tmp/tanuki-agent-state.sh"),
...
#[test]
fn install_omp_preserves_non_tanuki_file_with_pi_install_name() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let ext_dir = home.join(".omp/agent/extensions");
...
fn outdated_integrations_treat_missing_version_marker_as_legacy() {
    ...
    fs::write(&extension_path, "// installed by tanuki\n").unwrap();
    std::env::set_var("HOME", &home);
...
fn outdated_integrations_detect_previous_pi_version() {
    ...
    fs::write(
        &extension_path,
        "// TANUKI_INTEGRATION_ID=pi\n// TANUKI_INTEGRATION_VERSION=4\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn claude_v1_integration_status_is_outdated() {
    ...
    fs::write(
        &hook_path,
        "#!/bin/sh\n# TANUKI_INTEGRATION_ID=claude\n# TANUKI_INTEGRATION_VERSION=1\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
#[test]
fn uninstall_claude_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let claude_dir = home.join(".claude");
...
#[test]
fn uninstall_codex_removes_tanuki_hooks_and_leaves_config_alone() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let codex_dir = home.join(".codex");
    ...
    for event in COPILOT_REMOVED_LIFECYCLE_HOOK_EVENTS {
        if let Some(entries) = settings["hooks"].get(event) {
            assert!(
                ...
                "expected tanuki hooks.{event} entries to be removed"
            );
        }
    }
    assert!(settings["hooks"].get("sessionStart").is_none());
...
#[test]
fn uninstall_copilot_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let copilot_dir = home.join(".copilot");
...
#[test]
fn uninstall_devin_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let xdg_config = base.join("xdg");
    let devin_dir = xdg_config.join("devin");
...
fn install_droid_writes_hook_to_settings_and_cleans_legacy_hooks_json() {
    ...
    let legacy_hooks_json = json!({
        "hooks": {
            "SessionStart": [{"hooks": [{"type": "command", "command": legacy_command, "timeout": 10}]}],
            "PreToolUse": [{"matcher": "Read", "hooks": [{"type": "command", "command": "echo keep", "timeout": 10}]}]
        }
    });
    fs::write(
        droid_dir.join("hooks.json"),
        serde_json::to_string(&legacy_hooks_json).unwrap(),
    )
    .unwrap();
    fs::write(
...
#[test]
fn uninstall_droid_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let droid_dir = home.join(".factory");
    ...
    let hooks_json = json!({
        "hooks": {
            "SessionStart": [{"hooks": [
                {"type": "command", "command": command.clone(), "timeout": 10},
                {"type": "command", "command": "echo keep", "timeout": 10}
            ]}],
            "PreToolUse": [{"matcher": "Read", "hooks": [{"type": "command", "command": "echo read", "timeout": 10}]}]
        }
    });
    fs::write(
        droid_dir.join("hooks.json"),
    ...
    let settings_json = json!({
        "hooks": {
            ...
            "PostToolUse": [{"matcher": "Edit", "hooks": [{"type": "command", "command": "echo post", "timeout": 10}]}]
        }
    });
    fs::write(
        droid_dir.join("settings.json"),
...
fn install_hermes_is_idempotent_for_enabled_entry() {
    ...
    assert_eq!(config.matches("tanuki-agent-state").count(), 1);
...
fn install_hermes_is_idempotent_for_quoted_flat_plugin_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins:\n  - \"tanuki-agent-state\" # installed by tanuki\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn uninstall_hermes_removes_plugin_and_enabled_entry() {
    ...
    assert!(!config.contains("tanuki-agent-state"));
...
fn uninstall_hermes_removes_flow_plugin_list_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins: [other-plugin, tanuki-agent-state]\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn bundled_integration_assets_report_session_refs() {
    ...
    assert!(
        CODEX_HOOK_ASSET.contains("TANUKI_HOOK_INPUT_FILE")
    ...
    assert!(
        KIMI_HOOK_ASSET.contains("source\": \"tanuki:kimi")
            || KIMI_HOOK_ASSET.contains("--source tanuki:kimi")
    );
    assert!(
    ...
    assert!(DEVIN_HOOK_ASSET.contains("TANUKI_DEVIN_LIST_JSON"));
    ...
    assert!(KILO_PLUGIN_ASSET.contains("SOURCE = \"tanuki:kilo\""));
    ...
    assert!(QODERCLI_HOOK_ASSET.contains("TANUKI_PANE_ID"));
    ...
    assert!(CURSOR_HOOK_ASSET.contains("TANUKI_INTEGRATION_ID=cursor"));
    ...
    assert!(MASTRACODE_HOOK_ASSET.contains("TANUKI_INTEGRATION_ID=mastracode"));
    assert!(MASTRACODE_HOOK_ASSET.contains("TANUKI_INTEGRATION_VERSION=1"));
...
#[test]
fn uninstall_qodercli_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qoder_dir = base.join(".qoder");
    fs::create_dir_all(&qoder_dir).unwrap();
    ...
    assert!(session_start[0]
        ...
        .is_some_and(|command| {
            command.starts_with("bash ")
                && command.contains("tanuki-agent-state.sh")
...
#[test]
fn uninstall_cursor_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
...
fn install_mastracode_writes_hook_and_updates_hooks_json() {
    ...
    for (event, action) in MASTRACODE_HOOK_EVENTS {
        let entries = hooks.get(event).and_then(Value::as_array).unwrap();
        assert_eq!(entries.len(), 1, "{event} should have one Tanuki hook");
...
#[test]
fn uninstall_mastracode_removes_tanuki_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let original_home = std::env::var_os("HOME");
    std::env::set_var("HOME", &base);
