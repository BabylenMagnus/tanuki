...
fn apply_pane_terminal_env(cmd: &mut CommandBuilder) {
    // Each pane is rendered by tanuki's own terminal layer, not the outer terminal
...
fn apply_pane_launch_env(cmd: &mut CommandBuilder, launch_env: &PaneLaunchEnv) {
    ...
    cmd.env(crate::TANUKI_ENV_VAR, crate::TANUKI_ENV_VALUE);
    ...
    match &launch_env.identity {
        ...
        PaneLaunchIdentity::OmitPane => {
            cmd.env_remove(crate::integration::TANUKI_PANE_ID_ENV_VAR);
        }
    }
}
...
pub(crate) const WINDOWS_POWERSHELL_SHELL_INTEGRATION_COMMAND: &str = r"if ($null -eq $global:__TanukiOriginalPrompt) { $global:__TanukiOriginalPrompt = $function:prompt; function global:prompt { $out = @(& $global:__TanukiOriginalPrompt) -join ' '; $loc = $ExecutionContext.SessionState.Path.CurrentLocation; if ($loc.Provider.Name -eq 'FileSystem') { $esc = [string][char]27; $out += $esc + ']9;9;' + $loc.ProviderPath + $esc + '\' }; $out } }";
...
mod tests {
    ...
    async fn cwd_returns_accepted_report_without_rechecking_filesystem() {
        ...
        let cwd = std::env::temp_dir().join(format!(
            "tanuki-reported-cwd-cache-{}-{stamp}",
            std::process::id()
        ));
    ...
    fn windows_powershell_builder_injects_prompt_cwd_shell_integration() {
        ...
        assert!(
            script.contains("$global:__TanukiOriginalPrompt = $function:prompt"),
            "must wrap the profile-defined prompt: {script}"
        );
        assert!(
            script.contains("$null -eq $global:__TanukiOriginalPrompt"),
        ...
        let invoke_original = script
            .find("@(& $global:__TanukiOriginalPrompt)")
