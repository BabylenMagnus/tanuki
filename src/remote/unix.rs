...
pub(crate) const REMOTE_KEYBINDINGS_ENV_VAR: &str = "TANUKI_REMOTE_KEYBINDINGS";
...
pub(crate) fn run_remote(remote: RemoteLaunch) -> io::Result<()> {
    ...
    let program = std::env::args()
        .next()
        .unwrap_or_else(|| "tanuki".to_string());
    ...
    let prepared_remote = prepare_remote_tanuki(&remote_ssh, remote.live_handoff)?;
...
pub(crate) fn run_remote_client_bridge() -> io::Result<()> {
    ...
    let stream = UnixStream::connect(&socket_path).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "failed to connect to remote Tanuki client socket {}: {err}",
                socket_path.display()
            ),
        )
    })?;
...
fn ensure_remote_server_running() -> io::Result<()> {
    ...
    if crate::server::autodetect::is_server_listening() {
        ...
        return Err(io::Error::other(
            "remote tanuki server must restart before this bridge can attach; rerun `tanuki --remote` from an interactive terminal to approve stopping it",
        ));
    }
...
impl RemoteTanuki {
    fn for_platform(platform: RemotePlatform) -> Self {
        let install_suffix = ".local/bin/tanuki".to_string();
...
impl RemoteSsh {
    ...
    fn install_tanuki(&self, remote_tanuki: &RemoteTanuki, source_path: &Path) -> io::Result<()> {
        let output = self.sh_output(&remote_install_prepare_script(remote_tanuki))?;
...
fn remote_install_prepare_script(remote_tanuki: &RemoteTanuki) -> String {
    format!(
        r#"set -eu
...
"#,
        install_suffix = remote_tanuki.install_suffix
    )
}
...
fn prepare_remote_tanuki(
    ssh: &RemoteSsh,
    live_handoff_enabled: bool,
) -> io::Result<PreparedRemoteTanuki> {
    let platform = detect_remote_platform(ssh)?;
    let remote_tanuki = RemoteTanuki::for_platform(platform);
    let override_binary = remote_binary_override_path()?;
    let remote_binary_candidates = remote_binary_candidates(ssh, &remote_tanuki)?;
    ...
    if override_binary.is_none() {
        for candidate in &remote_binary_candidates {
            if remote_binary_matches(ssh, candidate).unwrap_or(false) {
                return Ok(PreparedRemoteTanuki {
                    remote_tanuki: candidate.clone(),
                    installed_or_replaced: false,
                    stop_after_install_approved: false,
                });
            }
        }
        if remote_binary_matches(ssh, &remote_tanuki)? {
            return Ok(PreparedRemoteTanuki {
                remote_tanuki,
                installed_or_replaced: false,
                stop_after_install_approved: false,
    ...
    if let Some(status_probe_tanuki) = remote_binary_candidates.first().or_else(|| {
        remote_binary_exists(ssh, &remote_tanuki)
            .ok()
            .and_then(|exists| exists.then_some(&remote_tanuki))
    }) {
    ...
    confirm_remote_install(
        ssh.target(),
        &remote_tanuki,
        &install_source_description(&remote_tanuki.platform, override_binary.as_deref()),
    )?;
    let source = resolve_install_source(&remote_tanuki.platform, override_binary)?;
    let install_result = ssh.install_tanuki(&remote_tanuki, &source.path);
    source.cleanup();
    install_result?;

    if !remote_binary_matches(ssh, &remote_tanuki)? {
        return Err(io::Error::other(format!(
            "installed remote tanuki at {}, but it did not report version {}",
            remote_tanuki.shell_path,
            current_version()
        )));
    }
    warn_if_remote_bin_not_on_path(ssh)?;

    Ok(PreparedRemoteTanuki {
        remote_tanuki,
...
fn remote_binary_candidates(
    ssh: &RemoteSsh,
    remote_tanuki: &RemoteTanuki,
) -> io::Result<Vec<RemoteTanuki>> {
    let mut candidates = Vec::new();

    if let Some(path_candidate) = remote_binary_on_path_any(ssh, remote_tanuki)? {
    ...
    let output = ssh.sh_output(&known_remote_binary_candidate_script(
        &remote_tanuki.platform,
    ))?;
    if !output.status.success() {
        return Err(command_failed("remote binary discovery failed", &output));
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    for candidate in remote_tanukis_from_path_discovery(remote_tanuki, &stdout) {
...
if [ -n "$home" ]; then
    emit "$home/.local/bin/tanuki"
fi
"#,
    );
    if platform.os == "macos" {
        script.push_str(
            r#"    emit "/opt/homebrew/bin/tanuki"
    emit "/usr/local/bin/tanuki"
"#,
        );
    } else if platform.os == "linux" {
        script.push_str(
            r#"    emit "/home/linuxbrew/.linuxbrew/bin/tanuki"
"#,
        );
    }
    script.push_str(
        r#"if [ -n "$home" ]; then
    emit "$home/.local/share/mise/installs/tanuki/$version/bin/tanuki"
    emit "$home/.local/share/mise/installs/tanuki/$version/tanuki"
    emit "$home/.local/share/mise/installs/github-ogulcancelik-tanuki/$version/tanuki"
    emit "$home/.nix-profile/bin/tanuki"
fi
if [ -n "$user" ]; then
    emit "/etc/profiles/per-user/$user/bin/tanuki"
fi
emit "/nix/var/nix/profiles/default/bin/tanuki"
emit "/run/current-system/sw/bin/tanuki"
"#,
    );
...
fn remote_binary_on_path_any(
    ssh: &RemoteSsh,
    remote_tanuki: &RemoteTanuki,
) -> io::Result<Option<RemoteTanuki>> {
    let output = ssh.user_shell_output("command -v tanuki")?;
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        if let Some(candidate) = remote_tanuki_from_path_discovery(remote_tanuki, &stdout) {
    ...
    let output = ssh.sh_output("command -v tanuki\n")?;
    if !output.status.success() {
        return Ok(None);
    }
...
fn remote_tanuki_from_path_discovery(
    remote_tanuki: &RemoteTanuki,
    stdout: &str,
) -> Option<RemoteTanuki> {
    stdout
        .lines()
        .find_map(|path| remote_tanuki_from_path(remote_tanuki, path))
}

fn remote_tanuki_from_path(remote_tanuki: &RemoteTanuki, path: &str) -> Option<RemoteTanuki> {
    let path = path.trim();
    if !path.starts_with('/') {
        return None;
    }
    ...
    Some(remote_tanuki.clone().with_shell_path(shell_quote(path)))
}

fn is_mise_shim_path(path: &str) -> bool {
    path.ends_with("/mise/shims/tanuki")
}

fn remote_binary_matches(ssh: &RemoteSsh, remote_tanuki: &RemoteTanuki) -> io::Result<bool> {
    let command = format!(
    ...
    Ok(version == format!("tanuki {}", current_version())
        && parse_client_status_json(status)
            .map(|status| status.protocol == CURRENT_PROTOCOL)
            .unwrap_or(false))
}

fn remote_binary_exists(ssh: &RemoteSsh, remote_tanuki: &RemoteTanuki) -> io::Result<bool> {
    let command = format!("test -x {}", remote_tanuki.shell_path);
...
fn install_source_description_for(
    ...
    if local_binary_can_seed_remote {
        "the current local tanuki binary".to_string()
    } else {
        format!(
            "the {} {} asset for {}",
            current_version(),
...
fn ensure_remote_server_ready(
    ...
    let status = remote_server_status(ssh, remote_tanuki)?;
    ...
    if live_handoff_enabled && live_handoff {
        match live_handoff_remote_server(ssh, remote_tanuki) {
            Ok(()) => return Ok(()),
            Err(err) => {
    ...
    if stop_after_install_approved {
        stop_remote_server(ssh, remote_tanuki)?;
        return Ok(());
    }
...
fn confirm_remote_install_with_running_server(
    ...
    let status = match remote_server_status(ssh, remote_tanuki) {
        Ok(status) => status,
        Err(err) => {
            if !io::stdin().is_terminal() {
                return Err(io::Error::other(format!(
                    "could not inspect the running remote tanuki server on {target} before installing: {err}; run from an interactive terminal to approve updating the remote binary"
                )));
            }
            eprintln!(
                "could not inspect the running remote tanuki server on {target} before installing: {err}"
            );
            eprint!("continue installing the remote tanuki binary? [y/N] ");
            io::stderr().flush()?;
            ...
            if answer != "y" && answer != "yes" {
                return Err(io::Error::new(
                    ...
                    "remote tanuki install cancelled",
                ));
            }
            return Ok(false);
        }
    ...
    if plan == RemoteInstallRunningServerPlan::KeepRunning {
        if io::stdin().is_terminal() {
            eprintln!("remote tanuki server on {target} is already compatible:");
            eprintln!("  server: v{}", version_label(version.as_deref()));
            eprintln!(
                "Tanuki will install {} without stopping the running remote server.",
                current_version()
            );
        }
        return Ok(false);
    ...
    if !io::stdin().is_terminal() {
        match plan {
            ...
            RemoteInstallRunningServerPlan::StopRequired(_) => {
                return Err(io::Error::other(format!(
                    "remote tanuki server on {target} is running v{}; run from an interactive terminal to approve stopping it for the update",
    ...
    if plan == RemoteInstallRunningServerPlan::LiveHandoff {
        eprintln!("remote tanuki server on {target} is currently running:");
        eprintln!("  server: v{}", version_label(version.as_deref()));
        eprintln!(
            "Tanuki will install {} and hand off live pane processes to the prepared server.",
            current_version()
        );
        return Ok(false);
    }
    ...
    eprintln!("  server: v{}", version_label(version.as_deref()));
    eprintln!(
        "To complete the remote update, Tanuki must stop the running remote server after installing."
    );
...
fn remote_server_status(
    ...
    let command = format!("{} status server --json", remote_tanuki.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn confirm_remote_server_stop(
    ...
    if !io::stdin().is_terminal() {
        if reason == RemoteServerRestartReason::ProtocolMismatch {
            return Err(io::Error::other(format!(
                "remote tanuki server on {target} must stop before this client can attach; run from an interactive terminal to approve stopping it"
            )));
        }

        eprintln!(
            "remote tanuki server on {target} is still running v{}; it will use {} after it restarts.",
    ...
    match reason {
        ...
        RemoteServerRestartReason::DaemonDetachMissing => {
            eprintln!(
                "the remote server was started by a tanuki build that may not survive SSH connection loss. restart it so network drops disconnect only this client."
            );
        }
        RemoteServerRestartReason::BinaryUpdated => {
            eprintln!(
                "the remote tanuki binary was installed or replaced. restart the remote server so it uses the prepared binary."
            );
        }
        RemoteServerRestartReason::VersionMismatch => {
            eprintln!(
                "the remote server is still running a different tanuki version. restart it so it uses the prepared binary."
            );
        }
    }
    ...
    if reason == RemoteServerRestartReason::ProtocolMismatch {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote tanuki server stop cancelled",
        ));
    }
...
fn live_handoff_remote_server(ssh: &RemoteSsh, remote_tanuki: &RemoteTanuki) -> io::Result<()> {
    let command = format!(
    ...
    eprintln!(
        "handed off the remote tanuki server on {}; reconnecting to the prepared server.",
        ssh.target()
    );
    Ok(())
}

fn stop_remote_server(ssh: &RemoteSsh, remote_tanuki: &RemoteTanuki) -> io::Result<()> {
    let command = format!("{} server stop", remote_tanuki.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
        return Err(command_failed("remote server stop failed", &output));
    }

    wait_for_remote_server_shutdown(ssh, remote_tanuki)?;
    eprintln!(
        "stopped the remote tanuki server on {}; it will restart when the remote client bridge attaches.",
        ssh.target()
    );
    Ok(())
}
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err(io::Error::new(
                ...
                format!(
                    "shutdown was requested, but the old remote tanuki server on {target} is still responding after {} seconds",
...
fn warn_if_remote_bin_not_on_path(ssh: &RemoteSsh) -> io::Result<()> {
    ...
    eprintln!(
        "tanuki: installed remote binary to ~/.local/bin/tanuki, but the remote shell does not resolve `tanuki` to that path"
    );
    Ok(())
}
...
fn remote_shell_resolves_managed_install(stdout: &str) -> bool {
    stdout
        ...
        .next()
        .map(str::trim)
        .is_some_and(|path| path.ends_with("/.local/bin/tanuki"))
}
...
fn preview_assets_for_build<'a>(
    ...
    let build = manifest.builds.get(build_id).ok_or_else(|| {
        io::Error::other(format!(
            "preview manifest no longer includes build {build_id}; run `tanuki update` locally or set {REMOTE_BINARY_ENV_VAR}=target/release/tanuki"
        ))
    })?;
...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        let build_id = crate::build_info::build_id().ok_or_else(|| {
            io::Error::other("preview client has no build id; set TANUKI_REMOTE_BINARY or install Tanuki on the remote manually")
        })?;
        ...
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "preview manifest has build {build_id} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/tanuki or install a matching Tanuki on the remote host manually"
            )));
        }
    ...
    let release = manifest.release_for_version(&current_version).ok_or_else(|| {
        io::Error::other(format!(
            "release manifest does not include tanuki {current_version}; build tanuki for {} or install it there manually",
            asset_key
        ))
    })?;
    if let Some(protocol) = release.protocol {
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "release manifest has tanuki {current_version} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/tanuki or install a matching tanuki on the remote host manually"
            )));
        }
    }
    release
        ...
        .ok_or_else(|| {
            io::Error::other(format!(
                "no {asset_key} binary in the release manifest for tanuki {current_version}"
            ))
        })
}
...
fn private_download_dir(asset_key: &str) -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "failed to create private tanuki remote download directory",
    ))
}
...
fn confirm_remote_install(
    ...
    if !io::stdin().is_terminal() {
        return Err(io::Error::other(format!(
            "matching remote tanuki {} is not installed at {}; run from an interactive terminal to approve installation",
            current_version(),
            remote_tanuki.shell_path
        )));
    }
    ...
    eprintln!(
        "matching tanuki {} is not installed on {target} for {}.",
        current_version(),
        remote_tanuki.platform.asset_key()
    );
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description, remote_tanuki.shell_path
    );
    io::stderr().flush()?;
    ...
    if answer == "n" || answer == "no" {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote tanuki installation cancelled",
        ));
    }
...
fn remote_bridge_command(remote_tanuki: &RemoteTanuki, session_name: &str) -> String {
    let mut command = format!("exec {}", remote_tanuki.shell_path);
...
fn private_ssh_config_dir() -> io::Result<PathBuf> {
    ...
    for base in bases {
        for attempt in 0..100 {
            let dir = base.join(format!("tanuki-ssh-{}-{attempt}", std::process::id()));
    ...
    Err(last_error.unwrap_or_else(|| {
        io::Error::new(
            io::ErrorKind::AlreadyExists,
            "failed to create private tanuki ssh config directory",
        )
    }))
}
...
fn run_client_process(
    ...
    let status = Command::new(exe)
        ...
        .env("TANUKI_RENDER_ENCODING", "terminal-ansi")
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable = tmpdir.join(format!(
        "tanuki-remote-{pid}-{target_clean}-{session_clean}.sock"
    ));
    ...
    let short_name = format!("tanuki-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "tanuki-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
        let remote_tanuki = RemoteTanuki::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
    ...
    fn managed_ssh_config_includes_user_config_then_fallback() {
        ...
        // tanuki's fallback transport settings are present...
        assert!(
        ...
        if let Some(home) = std::env::var_os("HOME") {
            ...
            if user_config.is_file() {
                ...
                assert!(
                    ...
                    "user config must be Included before tanuki's fallback: {contents}"
                );
            }
        }
    ...
    fn remote_install_stream_command_avoids_shell_c_wrapper() {
        let command = remote_install_stream_command("/home/a b/.local/bin/tanuki.tmp.123");

        assert_eq!(command, "tee '/home/a b/.local/bin/tanuki.tmp.123'");
    }
    ...
    fn remote_install_prepare_and_commit_scripts_quote_paths() {
        ...
        let prepare = remote_install_prepare_script(&remote_tanuki);

        assert!(prepare.contains("mkdir -p \"$dir\""));
        assert!(prepare.contains("printf '%s\\0%s\\0' \"$tmp\" \"$dest\""));
        assert_eq!(
            parse_remote_install_paths(b"/home/a b/tanuki.tmp.42\0/home/a b/tanuki\0").unwrap(),
            (
                "/home/a b/tanuki.tmp.42".to_string(),
                "/home/a b/tanuki".to_string()
            )
        );
        assert_eq!(
            parse_remote_install_paths(b"/home/a b\n/tanuki.tmp.42\0/home/a b\n/tanuki\0").unwrap(),
            (
                "/home/a b\n/tanuki.tmp.42".to_string(),
                "/home/a b\n/tanuki".to_string()
            )
        );
        assert_eq!(
            remote_install_commit_script("/home/a b/tanuki.tmp.42", "/home/a b/tanuki"),
            "set -eu\nchmod 755 '/home/a b/tanuki.tmp.42'\nmv '/home/a b/tanuki.tmp.42' '/home/a b/tanuki'\n"
        );
    }
    ...
    fn extract_remote_args_removes_space_form() {
        ...
        assert_eq!(cleaned, vec!["tanuki", "--help"]);
    ...
    fn extract_remote_args_removes_equals_form() {
        let args = vec!["tanuki".into(), "--remote=user@host".into()];
        let (cleaned, remote) = extract_remote_args(&args).unwrap();
        assert_eq!(cleaned, vec!["tanuki"]);
        let remote = remote.unwrap();
    ...
    fn extract_remote_args_accepts_explicit_handoff() {
        let args = vec!["tanuki".into(), "--remote=dev".into(), "--handoff".into()];
    ...
    fn extract_remote_args_preserves_handoff_without_remote() {
        let args = vec!["tanuki".into(), "update".into(), "--handoff".into()];
    ...
    fn extract_remote_args_rejects_remote_keybindings_without_remote() {
        let args = vec!["tanuki".into(), "--remote-keybindings=server".into()];
    ...
    fn extract_remote_args_requires_value() {
        let args = vec!["tanuki".into(), "--remote".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn reattach_command_includes_remote_and_session() {
        assert_eq!(
            ...
            "target/release/tanuki --remote user@host --session work"
        );
        assert_eq!(
            reattach_command(
                "tanuki",
        ...
        assert_eq!(
            ...
            "tanuki --remote host --remote-keybindings server"
        );
        assert_eq!(
            reattach_command(
                "tanuki",
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            remote_bridge_command(&remote_tanuki, crate::session::DEFAULT_SESSION_NAME),
            "exec \"$HOME/.local/bin/tanuki\" remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_path_binary() {
        ...
        let remote_tanuki = remote_tanuki_from_path_discovery(&remote_tanuki, "/usr/bin/tanuki\n")
            .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_tanuki, crate::session::DEFAULT_SESSION_NAME),
            "exec /usr/bin/tanuki remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_quotes_discovered_binary() {
        ...
        let remote_tanuki =
            remote_tanuki_from_path_discovery(&remote_tanuki, "/opt/tanuki bin/tanuki\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_tanuki, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/tanuki bin/tanuki' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_macos_path_binary() {
        ...
        let remote_tanuki =
            remote_tanuki_from_path_discovery(&remote_tanuki, "/opt/homebrew/bin/tanuki\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_tanuki, crate::session::DEFAULT_SESSION_NAME),
            "exec /opt/homebrew/bin/tanuki remote-client-bridge"
        );
        assert_eq!(remote_tanuki.platform.asset_key(), "macos-aarch64");
    }
    ...
    fn remote_path_discovery_reads_multiple_absolute_paths() {
        ...
        let candidates = remote_tanukis_from_path_discovery(
            &remote_tanuki,
            "/usr/bin/tanuki\nbin/tanuki\n /opt/tanuki bin/tanuki\n",
        );

        assert_eq!(candidates.len(), 2);
        assert_eq!(candidates[0].shell_path, "/usr/bin/tanuki");
        assert_eq!(candidates[1].shell_path, "'/opt/tanuki bin/tanuki'");
    }
    ...
    fn remote_path_discovery_ignores_mise_shims() {
        ...
        let candidates = remote_tanukis_from_path_discovery(
            &remote_tanuki,
            "/home/can/.local/share/mise/shims/tanuki\n/home/can/.local/share/mise/installs/tanuki/0.7.1/bin/tanuki\n",
        );
    ...
    fn known_remote_binary_candidate_script_includes_mise_and_nix_paths() {
        ...
        assert!(script.contains("emit \"$home/.local/bin/tanuki\""));
        assert!(!script.contains("mise/shims/tanuki"));
        assert!(script.contains(&format!("version={}", shell_quote(&current_version()))));
        assert!(
            script.contains("emit \"$home/.local/share/mise/installs/tanuki/$version/bin/tanuki\"")
        );
        assert!(script.contains("emit \"$home/.local/share/mise/installs/tanuki/$version/tanuki\""));
        assert!(script.contains(
            "emit \"$home/.local/share/mise/installs/github-ogulcancelik-tanuki/$version/tanuki\""
        ));
        assert!(script.contains("emit \"$home/.nix-profile/bin/tanuki\""));
        assert!(script.contains("emit \"/etc/profiles/per-user/$user/bin/tanuki\""));
        assert!(script.contains("emit \"/run/current-system/sw/bin/tanuki\""));
        assert!(script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/tanuki\""));
        assert!(!script.contains("emit \"/opt/homebrew/bin/tanuki\""));
    }
    ...
    fn known_remote_binary_candidate_script_includes_macos_homebrew_paths() {
        ...
        assert!(script.contains("emit \"/opt/homebrew/bin/tanuki\""));
        assert!(script.contains("emit \"/usr/local/bin/tanuki\""));
        assert!(!script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/tanuki\""));
    }
    ...
    fn remote_path_discovery_quotes_single_quotes_in_discovered_binary() {
        ...
        let remote_tanuki =
            remote_tanuki_from_path_discovery(&remote_tanuki, "/opt/tanuki's/bin/tanuki\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_tanuki, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/tanuki'\\''s/bin/tanuki' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_ignores_relative_paths() {
        ...
        let remote_tanuki = remote_tanuki_from_path_discovery(&remote_tanuki, "bin/tanuki\n");

        assert!(remote_tanuki.is_none());
    }
    ...
    fn remote_path_discovery_ignores_empty_output() {
        ...
        let remote_tanuki = remote_tanuki_from_path_discovery(&remote_tanuki, "\n");
    ...
    fn remote_shell_path_warning_accepts_managed_install() {
        assert!(remote_shell_resolves_managed_install(
            "/home/can/.local/bin/tanuki\n"
        ));
        assert!(remote_shell_resolves_managed_install(
            "/Users/can/.local/bin/tanuki\n"
        ));
        assert!(!remote_shell_resolves_managed_install(
    ...
    fn parse_client_status_json_reads_protocol() {
        assert_eq!(
            parse_client_status_json(r#"{"version":"x","protocol":8,"binary":"/bin/tanuki"}"#)
    ...
    fn install_source_description_uses_local_binary_when_allowed() {
        ...
        assert_eq!(
            ...
            "the current local tanuki binary"
        );
    }
    ...
    fn resolve_install_source_uses_override_binary_without_temporary_cleanup() {
        ...
        let source = resolve_install_source(&platform, Some(PathBuf::from("/tmp/tanuki-aarch64")))
            .expect("override source");
        assert_eq!(source.path, PathBuf::from("/tmp/tanuki-aarch64"));
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("tanuki-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "tanuki-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
