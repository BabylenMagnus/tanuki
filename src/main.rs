...
pub(crate) const TANUKI_ENV_VAR: &str = "TANUKI_ENV";
pub(crate) const TANUKI_ENV_VALUE: &str = "1";
const NESTED_TANUKI_MESSAGES: [&str; 6] = [
    ...
    "recursive tanukiing is disabled. somewhere, a call stack breathes a sigh of relief.",
    "recursive descent denied. there is, in fact, such a thing as too much tanuki.",
...
fn init_logging() {
    crate::logging::init_file_logging("tanuki.log");
}

const DEFAULT_CONFIG: &str = r##"# tanuki configuration
# Place this file at ~/.config/tanuki/config.toml
...
# Follow host terminal light/dark appearance and switch Tanuki UI themes.
...
# "current" for Tanuki's process directory, or a fixed path such as "~/Projects".
# new_cwd = "follow"

[update]
# Update channel used by background version checks and `tanuki update`.
...
# Check tanuki.example for new Tanuki versions in the background.
# version_check = true

# Check tanuki.example for remote agent-detection manifest updates in the background.
# manifest_check = true
...
# remote_image_paste = "ctrl+v" # only active in tanuki --remote; empty disables raw-key image paste
# new_tab = "prefix+c"
...
# Terminal width at or below which Tanuki uses the mobile single-column layout.
# Increase this for foldables, tablets, or wide phone terminals.
# mobile_width_threshold = 64

# Capture mouse input for Tanuki's mouse UI.
...
# "auto" draws Tanuki's own cursor on native Windows builds and WSL to avoid ConPTY cursor flicker, and uses the native terminal cursor elsewhere.
# "native" always uses the outer terminal cursor. "drawn" always draws Tanuki's cursor as terminal cell content.
# host_cursor = "auto"

# Optional modifier that forwards right-click hold/drag gestures to pane apps instead of opening Tanuki's pane menu.
...
# Set false to reduce visible flashing when switching back to Tanuki.
...
# a Tanuki server restart. Requires official integrations that report session refs.
# resume_agents_on_restore = true

[remote]
# Whether tanuki manages the ssh config used for `tanuki --remote`.
# When true (default), tanuki runs remote ssh through a generated config that
...
# still win) to survive idle network/NAT timeouts. Tanuki also uses a private
...
# force keepalive or multiplexing off, it only stops tanuki from adding its own.
# manage_ssh_config = true

[experimental]
# Allow launching tanuki from inside a tanuki-managed pane.
# allow_nested = false
...
fn should_block_nested_for_env(config: &config::Config, tanuki_env: Option<&str>) -> bool {
    !config.experimental.allow_nested && tanuki_env == Some(TANUKI_ENV_VALUE)
}
...
fn random_nested_message() -> &'static str {
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    let index = (nanos ^ (std::process::id() as usize)) % NESTED_TANUKI_MESSAGES.len();
    NESTED_TANUKI_MESSAGES[index]
}

fn exit_if_nested_disabled(config: &config::Config) {
    if should_block_nested(config) {
        eprintln!("\x1b[1merror:\x1b[0m nested tanuki is disabled by default.");
...
fn main() -> io::Result<()> {
    ...
    let args = match session::configure_from_args(&raw_args) {
        ...
        Err(err) => {
            eprintln!("error: {err}");
            eprintln!("run 'tanuki --help' for usage");
            std::process::exit(2);
        }
    };
    ...
    let (args, cloud_target) = match remote::extract_cloud_args(&args) {
        Ok(parsed) => parsed,
        Err(err) => {
            eprintln!("error: {err}");
            eprintln!("run 'tanuki --help' for usage");
    ...
    if cloud_target.is_some() && remote_launch.is_some() {
        eprintln!("error: --cloud cannot be combined with --remote");
        eprintln!("run 'tanuki --help' for usage");
        std::process::exit(2);
    }
    ...
    {
        eprintln!("error: --cloud can only be used with the default launch command");
        eprintln!("run 'tanuki --help' for usage");
        std::process::exit(2);
    }
    ...
    if args.get(1).map(|s| s.as_str()) == Some("update") {
        let options = match update::parse_self_update_args(&args[2..]) {
            ...
            Err(err) => {
                eprintln!("{err}");
                eprintln!("usage: tanuki update [--handoff]");
                std::process::exit(2);
            }
        };
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        println!("tanuki — terminal workspace manager for AI coding agents");
        println!();
        println!("Usage: tanuki [options]");
        ...
        println!("       tanuki update [--handoff]");
        println!("       tanuki channel set <stable|preview>");
        println!("       tanuki server stop");
        println!("       tanuki server reload-config");
        println!("       tanuki api <subcommand> ...");
        println!("       tanuki completion <shell>");
        println!("       tanuki config <subcommand> ...");
        println!("       tanuki channel <subcommand> ...");
        println!("       tanuki workspace <subcommand> ...");
        println!("       tanuki worktree <subcommand> ...");
        println!("       tanuki tab <subcommand> ...");
        println!("       tanuki notification <subcommand> ...");
        println!("       tanuki agent <subcommand> ...");
        println!("       tanuki pane <subcommand> ...");
        println!("       tanuki session <subcommand> ...");
        println!("       tanuki integration <subcommand> ...");
        println!();
        println!("Common commands:");
        for (command, description) in [
            ("tanuki", "Launch or attach to the persistent session"),
            (
                "tanuki status [server|client]",
        ...
        println!("  {:<32} Run as headless server", "tanuki server");
        println!();
        println!("Options:");
        println!("  --no-session        Run monolithically (no server/client, escape hatch)");
        println!("  --session <name>    Use or create a named persistent session");
        println!("  --remote <target>   Attach through SSH to a remote Tanuki server");
        println!("  --remote-keybindings <local|server>");
        println!("                      Keybindings for --remote app attach (default: local)");
        println!("  --cloud <token-id>  Attach to another of your own paired devices' hosted");
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("tanuki {}", crate::build_info::version());
        return Ok(());
    }
    ...
    if let Some(cloud_target) = cloud_target {
        // A cloud viewer attaches to a session on another of the caller's
        // own paired devices -- there is no local server to spawn/attach
        // to here, so this deliberately bypasses `auto_detect_launch`.
        if let Err(err) = client::run_cloud_viewer(cloud_target) {
            eprintln!("error: {err}");
            std::process::exit(1);
        }
        return Ok(());
    ...
    let _api_server = match api::start_server_with_capabilities(api_tx, event_hub.clone(), None) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: tanuki is already running");
...
mod tests {
    ...
    #[test]
    fn nested_tanuki_blocks_when_env_is_set() {
    ...
    #[test]
    fn nested_tanuki_does_not_block_without_env() {
        let config = config::Config::default();
    ...
    fn random_nested_message_comes_from_known_set() {
        ...
        assert!(NESTED_TANUKI_MESSAGES.contains(&message));
    }

    #[test]
    fn nested_message_strings_no_longer_repeat_tanuki_prefix() {
        assert!(NESTED_TANUKI_MESSAGES
            .iter()
            .all(|message| !message.starts_with("tanuki:")));
    }
}
