...
fn agent_panel_sort_label(sort: AgentPanelSort) -> String {
    match sort {
        AgentPanelSort::Spaces => t!("sidebar.sort.grouped").to_string(),
        AgentPanelSort::Priority => t!("sidebar.sort.priority").to_string(),
    }
}
...
"##,
    ...
    async fn all_workspaces_agent_panel_entries_use_live_root_runtime_cwd_for_workspace_label() {
        let unique = format!(
            "tanuki-agent-panel-runtime-cwd-{}-{}",
            std::process::id(),
        ...
        assert_eq!(primary_label, "tanuki");
    }
    ...
    fn grouped_child_label_uses_short_branch_for_auto_named_workspace() {
        assert_eq!(
            grouped_child_display_label("tanuki-issue", Some("worktree/issue-137"), false),
            "issue-137"
        );
    }
    ...
    fn parent_workspace_row_stays_clickable_when_grouped() {
        ...
        app.workspaces = vec![
            workspace_with_worktree_space("main", Some("repo-key"), "/repo/tanuki"),
            workspace_with_worktree_space("issue", Some("repo-key"), "/repo/tanuki-issue"),
        ];
    ...
    fn space_row_gap_preserves_compact_worktree_children() {
        ...
        app.workspaces = vec![
            workspace_with_worktree_space("main", Some("repo-key"), "/repo/tanuki"),
            workspace_with_worktree_space("issue", Some("repo-key"), "/repo/tanuki-issue"),
            workspace_with_worktree_space("review", Some("repo-key"), "/repo/tanuki-review"),
    ...
    fn compact_space_group_scroll_clamps_when_all_entries_fit() {
        ...
        app.workspaces = vec![
            workspace_with_worktree_space("main", Some("repo-key"), "/repo/tanuki"),
            workspace_with_worktree_space("one", Some("repo-key"), "/repo/tanuki-one"),
            workspace_with_worktree_space("two", Some("repo-key"), "/repo/tanuki-two"),
        ];
        let area = Rect::new(0, 0, 30, 20);
