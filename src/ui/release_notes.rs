...
pub(super) fn render_release_notes_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let subtitle = if notes.preview {
        t!("release_notes.update_ready")
    } else {
        t!("release_notes.whats_new")
    };
    frame.render_widget(
        Paragraph::new(subtitle.to_string()).style(Style::default().fg(app.palette.overlay1)),
...
pub(super) fn render_product_announcement_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let subtitle = if announcement.preview {
        t!("release_notes.announcement_preview")
    } else {
        t!("release_notes.announcement")
    };
    frame.render_widget(
...
fn release_notes_preview_line_entries<'a>(
    ...
    let update_ready_label = format!(" {}", t!("release_notes.update_ready"));
    let title_width = 2 + update_ready_label.chars().count();
    ...
    vec![
        (
            ...
            Line::from(vec![
                ...
                Span::styled(update_ready_label, title_style),
            ]),
        ),
...
pub(crate) fn release_notes_close_button_rect(area: Rect) -> Rect {
    let close = t!("release_notes.close").to_string();
...
mod tests {
    ...
    fn release_notes_inline_code_spans_are_styled_without_backticks() {
        ...
        let lines = release_notes_lines("- `tanuki pane run ...` now works", &palette);

        assert_eq!(lines.len(), 1);
        assert_eq!(line_text(&lines[0].1), " • tanuki pane run ... now works");
        assert_eq!(lines[0].1.spans[1].content.as_ref(), "tanuki pane run ...");
    ...
    fn release_notes_preview_lines_show_update_steps() {
        let palette = Palette::catppuccin();
        let lines = release_notes_preview_line_entries("tanuki update", &palette)
            .into_iter()
            .map(|(_, line)| line)
            .collect::<Vec<_>>();
        ...
        assert_eq!(lines[1].spans[2].content.as_ref(), "tanuki update");
    ...
    fn release_notes_preview_display_is_part_of_the_scrollable_notes_body() {
        ...
        let lines = release_notes_display_lines(&notes, "tanuki update", &palette);
