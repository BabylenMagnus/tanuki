...
impl App {
    ...
    > {
        ...
        let space = existing_membership
            ...
            .ok_or_else(|| {
                "Tanuki worktree actions require a workspace inside a Git work tree.".to_string()
            })?;
    ...
    pub(crate) fn open_remove_linked_worktree_confirmation(&mut self, ws_idx: usize) {
            ...
            .is_some_and(|space| space.is_linked_worktree)
        {
            self.state.config_diagnostic =
                Some("This workspace is not a Tanuki-managed worktree checkout.".into());
            return;
        }
...
mod tests {
    ...
    fn create_committed_repo(name: &str) -> std::path::PathBuf {
        ...
        run_git(&repo, &["config", "user.email", "tanuki@example.invalid"]);
        run_git(&repo, &["config", "user.name", "Tanuki Test"]);
    ...
    fn worktree_create_replaces_prefilled_branch_on_paste_and_syncs_state() {
        ...
        app.state.worktree_create = Some(WorktreeCreateState {
            ...
            checkout_path: "/repo/tanuki-generated-branch".into(),
            error: None,
            creating: false,
        });
    ...
    fn open_selected_existing_worktree_focuses_already_open_workspace() {
        ...
        app.state.workspaces[1].identity_cwd = "/repo/tanuki-issue".into();
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/repo/tanuki-issue")
        );
    ...
    fn sync_worktree_branch_updates_derived_path() {
        ...
        app.state.worktree_create = Some(WorktreeCreateState {
            source_workspace_id: "source".into(),
            source_checkout_path: std::path::PathBuf::from("/repo/tanuki"),
            source_existing_membership: None,
            source_repo_root: std::path::PathBuf::from("/repo/tanuki"),
            repo_key: "repo-key".into(),
            repo_name: "tanuki".into(),
            branch: "old".into(),
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/w/tanuki/issue-137")
        );
    ...
    fn worktree_create_enter_submits_through_api_path() {
        ...
        let checkout_path =
            crate::worktree::default_checkout_path(&app.state.worktree_directory, "tanuki", branch);
    ...
    async fn ui_worktree_create_emits_api_parity_events_after_membership_is_committed() {
        ...
        let checkout = crate::worktree::default_checkout_path(&worktree_root, "tanuki", branch);
        run_git(
            &repo,
            &[
                "worktree",
        ...
        let manifest_path = plugin_root.join("tanuki-plugin.toml");
    ...
    fn dirty_worktree_remove_failure_requests_force_confirmation() {
        let path = std::path::PathBuf::from("/w/tanuki/dirty");
        let mut app = app_for_worktree_tests();
        app.state.worktree_remove = Some(WorktreeRemoveState {
            workspace_id: "ws".into(),
            repo_root: std::path::PathBuf::from("/repo/tanuki"),
            path: path.clone(),
            error: None,
            removing: true,
            force_confirmation: false,
        ...
        app.handle_worktree_remove_finished(WorktreeRemoveResult {
            ...
            api_request: None,
            result: Err(
                "fatal: '/w/tanuki/dirty' contains modified or untracked files, use --force to delete it"
                    .into(),
            ),
        });
    ...
    fn non_dirty_worktree_remove_failure_keeps_error_message() {
        let path = std::path::PathBuf::from("/w/tanuki/missing");
        let mut app = app_for_worktree_tests();
        app.state.worktree_remove = Some(WorktreeRemoveState {
            workspace_id: "ws".into(),
            repo_root: std::path::PathBuf::from("/repo/tanuki"),
        ...
        app.handle_worktree_remove_finished(WorktreeRemoveResult {
            ...
            forced: false,
            api_request: None,
            result: Err("fatal: '/w/tanuki/missing' is not a working tree".into()),
        });
        ...
        assert_eq!(
            remove.error,
            Some("fatal: '/w/tanuki/missing' is not a working tree".into())
        );
    }
    ...
    fn worktree_remove_finished_focuses_parent_workspace() {
        let mut app = app_for_worktree_tests();
        let checkout = std::path::PathBuf::from("/repo/tanuki-issue");
        app.state.workspaces = vec![
        ...
        app.state.workspaces[2].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            ...
            label: "tanuki".into(),
            repo_root: "/repo/tanuki".into(),
            checkout_path: "/repo/tanuki-sibling".into(),
            is_linked_worktree: true,
        });
