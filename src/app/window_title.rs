...
//!
//! `tanuki_terminal` is a terminal emulator, so `OSC 0`/`OSC 2` written by a pane
//! stops at it and never reaches the terminal it itself runs in. Without this the
...
impl App {
    ...
    /// Whether `ui.window_title` asks tanuki_terminal to own the outer terminal
    /// title at all. When it does not, the host leaves whatever the shell or
    /// `ssh` set.
