...
mod tests {
    ...
    fn unique_temp_path(name: &str) -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        std::env::temp_dir().join(format!("tanuki-{name}-{}-{nanos}", std::process::id()))
    }
    ...
    fn write_manifest(root: &std::path::Path) -> std::path::PathBuf {
        ...
        let manifest = root.join("tanuki-plugin.toml");
        std::fs::write(
            &manifest,
            r#"
...
"#,
        ...
        let response = app.handle_api_request(Request {
            ...
            method: Method::PluginLink(PluginLinkParams {
                ...
                source: Some(PluginSourceInfo {
                    ...
                    repo: Some("tanuki-plugin-examples".into()),
    ...
    #[test]
    fn link_rejects_invalid_min_tanuki_versions() {
        let cases = [
            (
                "plugin-missing-min-tanuki",
                r#"
id = "example.missing-min-tanuki"
name = "Missing Min Tanuki"
version = "0.1.0"
platforms = ["linux", "macos", "windows"]
"#,
                "invalid_plugin_min_tanuki_version",
            ),
            (
                "plugin-invalid-min-tanuki",
                r#"
id = "example.invalid-min-tanuki"
name = "Invalid Min Tanuki"
version = "0.1.0"
min_tanuki_version = "soon"
platforms = ["linux", "macos", "windows"]
"#,
                "invalid_plugin_min_tanuki_version",
            ),
            (
                "plugin-future-min-tanuki",
                r#"
id = "example.future-min-tanuki"
name = "Future Min Tanuki"
version = "0.1.0"
min_tanuki_version = "999.0.0"
platforms = ["linux", "macos", "windows"]
"#,
                "plugin_requires_newer_tanuki",
            ),
            (
...
id = "board"
title = "Plugin Board"
command = ["sh", "-c", "printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \"$PWD\" \"$TANUKI_PLUGIN_ID\" \"$TANUKI_PLUGIN_ENTRYPOINT_ID\" \"$TANUKI_WORKSPACE_ID\" \"$TANUKI_PANE_ID\" \"$TANUKI_BIN_PATH\" \"$TANUKI_PLUGIN_CONTEXT_JSON\" \"${{TANUKI_CELL_WIDTH_PX-unset}}\" \"${{TANUKI_CELL_HEIGHT_PX-unset}}\" > {}"]
"#,
                capture.display()
            ),
        );
        ...
        let open = app.handle_api_request(Request {
            ...
            method: Method::PluginPaneOpen(PluginPaneOpenParams {
                ...
                env: std::collections::HashMap::from([
                    ("TANUKI_PLUGIN_ID".to_string(), "spoofed-plugin".to_string()),
                    (
                        "TANUKI_PLUGIN_ENTRYPOINT_ID".to_string(),
                        "spoofed-entrypoint".to_string(),
                    ),
                    (
                        "TANUKI_PLUGIN_CONTEXT_JSON".to_string(),
                        "{\"spoofed\":true}".to_string(),
                    ),
                    (
                        "TANUKI_BIN_PATH".to_string(),
                        "/tmp/spoofed-tanuki".to_string(),
                    ),
                ]),
            }),
        });
        ...
        assert_ne!(bin_path, "/tmp/spoofed-tanuki");
        assert_eq!(
            bin_path,
            std::env::current_exe()
...
id = "board"
title = "Plugin Board"
command = ["sh", "-c", "printf '%s\n%s\n%s\n' \"$TANUKI_PLUGIN_ROOT\" \"$TANUKI_PLUGIN_CONFIG_DIR\" \"$TANUKI_PLUGIN_STATE_DIR\" > {}"]
"#,
                capture.display()
            ),
        );
        ...
        let open = app.handle_api_request(Request {
            ...
            method: Method::PluginPaneOpen(PluginPaneOpenParams {
                ...
                env: std::collections::HashMap::from([
                    (
                        "TANUKI_PLUGIN_ROOT".to_string(),
                        "/tmp/spoofed-root".to_string(),
                    ),
                    (
                        "TANUKI_PLUGIN_CONFIG_DIR".to_string(),
                        "/tmp/spoofed-config".to_string(),
                    ),
                    (
                        "TANUKI_PLUGIN_STATE_DIR".to_string(),
...
width = "80%"
height = "40%"
command = ["sh", "-c", "printf %s ${{TANUKI_PANE_ID-unset}} > '{}'; sleep 1"]
"#,
            env_capture.display()
        );
        write_manifest_content(&root, &manifest);
...
id = "run"
title = "Run"
command = ["sh", "-c", "printf '%s' \"$TANUKI_PLUGIN_ACTION_ID\""]
"#,
        );
        link_manifest(&mut app, &root);
...
id = "run"
title = "Run"
command = ["sh", "-c", "printf '%s\n%s\n%s' \"$TANUKI_PLUGIN_ROOT\" \"$TANUKI_PLUGIN_CONFIG_DIR\" \"$TANUKI_PLUGIN_STATE_DIR\""]
"#,
        );
        link_manifest(&mut app, &root);
...
[[startup]]
command = ["sh", "-c", "printf '%s:%s' \"$TANUKI_PLUGIN_ID\" \"$TANUKI_PLUGIN_EVENT\" > {}"]
"#,
                capture.display()
            ),
        );
...
[[events]]
on = "worktree.created"
command = ["sh", "-c", "printf '%s' \"$TANUKI_PLUGIN_CONTEXT_JSON\" > {}"]
"#,
                capture.display()
            ),
        );
    ...
    fn closed_event_context_uses_closed_target_ids() {
        ...
        app.state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "tanuki".into(),
            repo_root: "/repo/tanuki".into(),
            checkout_path: "/repo/tanuki-issue".into(),
            is_linked_worktree: true,
        });
        let workspace = app.workspace_info(0);
        let worktree = crate::api::schema::WorktreeInfo {
            path: "/repo/tanuki-issue".into(),
        ...
        app.state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            ...
            label: "tanuki".into(),
            repo_root: "/repo/tanuki".into(),
            checkout_path: "/repo/tanuki-other".into(),
            is_linked_worktree: true,
        });
...
id = "open"
title = "Open link"
command = ["sh", "-c", "printf '%s|%s' \"$TANUKI_PLUGIN_LINK_HANDLER_ID\" \"$TANUKI_PLUGIN_CLICKED_URL\""]
...
"#,
        ...
        assert_eq!(worktree.repo_name, "tanuki");
        assert_eq!(worktree.repo_root, "/repo/tanuki");
        assert_eq!(worktree.checkout_path, "/repo/tanuki-issue");
