...
struct RawPluginManifest {
    ...
    version: String,
    #[serde(default)]
    min_tanuki_version: Option<String>,
    #[serde(default)]
...
pub(crate) fn load_plugin_manifest(
    ...
    let min_tanuki_version = validate_min_tanuki_version(raw.min_tanuki_version.as_deref())?;
    let description = raw
        .description
...
fn validate_min_tanuki_version(value: Option<&str>) -> Result<String, (&'static str, String)> {
    let Some(value) = value else {
        return Err((
            "invalid_plugin_min_tanuki_version",
            "plugin min_tanuki_version is required".to_string(),
        ));
    };
    let value = non_empty_trimmed(
        value,
        "invalid_plugin_min_tanuki_version",
        "plugin min_tanuki_version is required",
    )?;
    let required = crate::update::Version::parse(&value).ok_or_else(|| {
        (
            "invalid_plugin_min_tanuki_version",
            format!(
                "plugin min_tanuki_version must be a semantic version like {}",
    ...
    if required > current {
        return Err((
            ...
            format!("plugin requires Tanuki {required} or newer; current Tanuki is {current}"),
        ));
    }
