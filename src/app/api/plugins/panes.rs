...
impl App {
    ...
    fn plugin_pane_launch_env(
        ...
        env.push(("TANUKI_ENV".to_string(), "1".to_string()));
        env.push(("TANUKI_PLUGIN_ID".to_string(), plugin.plugin_id.clone()));
        env.push((
            "TANUKI_PLUGIN_ENTRYPOINT_ID".to_string(),
            entrypoint.to_string(),
        ));
        env.push(("TANUKI_PLUGIN_CONTEXT_JSON".to_string(), context_json));
...
fn plugin_pane_protected_env_key(key: &str) -> bool {
    matches!(
        ...
        crate::api::SOCKET_PATH_ENV_VAR
            ...
            | "TANUKI_PLUGIN_ID"
            | "TANUKI_PLUGIN_ROOT"
            | "TANUKI_PLUGIN_CONFIG_DIR"
            | "TANUKI_PLUGIN_STATE_DIR"
            | "TANUKI_PLUGIN_ENTRYPOINT_ID"
            | "TANUKI_PLUGIN_CONTEXT_JSON"
            | "TANUKI_BIN_PATH"
    )
}
