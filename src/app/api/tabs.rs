...
impl App {
    ...
    pub(super) fn handle_tab_focus(&mut self, id: String, target: TabTarget) -> String {
        ...
        // `switch_workspace_tab` sets `state.pending_full_redraw` itself when
        // the active workspace/tab actually changes, which every render loop
        // drains into a full redraw -- avoids the staircase redraw that
        // diffing a large-area content change produces.
        self.state.switch_workspace_tab(ws_idx, tab_idx);
