...
impl App {
    ...
    /// key-handling path as monolithic tanuki so they are re-encoded for the
...
mod tests {
    ...
    fn unique_temp_path(name: &str) -> std::path::PathBuf {
        let stamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("tanuki-{name}-{}-{stamp}", std::process::id()))
    }
    ...
    #[test]
    fn notification_show_api_creates_tanuki_toast_with_position() {
        let mut app = test_app();
        app.state.toast_config.delivery = crate::config::ToastDelivery::Tanuki;
        ...
        assert_eq!(
            toast.position,
            Some(crate::config::ToastTanukiPosition::TopLeft)
        );
        assert!(app.toast_deadline.is_some());
    }

    #[test]
    fn notification_show_api_tanuki_toast_expires() {
        let mut app = test_app();
        app.state.toast_config.delivery = crate::config::ToastDelivery::Tanuki;
    ...
    fn startup_does_not_restore_update_available_from_older_saved_notes() {
        ...
        crate::release_notes::save_pending("0.0.1", "### Changed\n- One").unwrap();
    ...
    fn reload_config_invalid_sidebar_bounds_keeps_previous_ui_and_returns_partial() {
        ...
        assert_eq!(
            ...
            Some("config.toml; tanuki config check")
        );
    ...
    fn reload_config_applies_known_sibling_and_summarizes_unknown_key() {
        ...
        assert_eq!(
            app.state.config_diagnostic.as_deref(),
            Some("config.toml has unknown keys; tanuki config check")
        );
    ...
    fn reload_config_keeps_current_state_on_invalid_toml() {
        ...
        assert!(app
            ...
            .as_deref()
            .is_some_and(|message| {
                message == "config.toml invalid; keeping current config; tanuki config check"
            }));
        assert!(app.state.toast.is_none());
    ...
    fn workspace_creation_in_navigate_mode_uses_selected_workspace_seed_cwd() {
        let mut app = test_app();
        let mut first = Workspace::test_new("tanuki");
        first.identity_cwd = std::path::PathBuf::from("/tmp/tanuki");
    ...
    fn new_terminal_cwd_follow_uses_source_cwd() {
        let cwd = creation::resolve_new_terminal_cwd(
            ...
            Some(std::path::PathBuf::from("/tmp/tanuki-source")),
        );

        assert_eq!(cwd, std::path::PathBuf::from("/tmp/tanuki-source"));
    }
    ...
    fn new_terminal_cwd_path_uses_configured_path() {
        let cwd = creation::resolve_new_terminal_cwd(
            &crate::config::NewTerminalCwdConfig::Path("/tmp/tanuki-fixed".into()),
            Some(std::path::PathBuf::from("/tmp/tanuki-source")),
        );

        assert_eq!(cwd, std::path::PathBuf::from("/tmp/tanuki-fixed"));
    }
    ...
    fn pane_close_request_requires_confirmation_before_closing_parent_worktree_group() {
        ...
        child.worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            ...
            label: "tanuki".into(),
            repo_root: "/repo/tanuki".into(),
            checkout_path: "/repo/tanuki-child".into(),
