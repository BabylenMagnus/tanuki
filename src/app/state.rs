...
impl SettingsSection {
    ...
    pub fn translated_label(self) -> String {
        ...
        match self {
            ...
            Self::Indicators => t!("settings.tab.indicators").to_string(),
...
/// A keybind action exposed for editing in the Keybinds settings tab.
///
/// Covers every plain `ActionKeybinds` field on `Keybinds`. Deliberately
/// excludes: `navigate_*` (navigate-mode-only, must stay a bare unmodified
/// key per the app's own binding rules — a different validation story),
/// the indexed fields `focus_agent`/`switch_tab`/`switch_workspace`
/// (`Vec<IndexedKeybind>`, a `1..9` range shorthand — different editing
/// model), and `custom_commands` (a different struct entirely, has its own
/// command/type/popup-size fields, not a bare binding).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum KeybindSetting {
    PreviousAgent,
    NextAgent,
    PreviousWorkspace,
    NextWorkspace,
...
impl KeybindSetting {
    ...
    pub(crate) fn label(self) -> &'static str {
        match self {
            ...
            Self::PreviousWorkspace => "previous space",
            Self::NextWorkspace => "next space",
            Self::Help => "help overlay",
            Self::Settings => "open settings",
            Self::WorkspacePicker => "space picker",
            Self::NewWorkspace => "new space",
            Self::NewWorktree => "new worktree",
            Self::OpenWorktree => "open worktree",
            Self::RemoveWorktree => "remove worktree",
            Self::RenameWorkspace => "rename space",
            Self::CloseWorkspace => "close space",
            Self::Goto => "navigator / goto",
            Self::Detach => "detach client",
    ...
    pub(crate) fn current_binding(self, keybinds: &crate::config::Keybinds) -> Option<String> {
        match self {
            Self::PreviousAgent => keybinds.previous_agent.label(),
            Self::NextAgent => keybinds.next_agent.label(),
            Self::PreviousWorkspace => keybinds.previous_workspace.label(),
            Self::NextWorkspace => keybinds.next_workspace.label(),
            Self::Help => keybinds.help.label(),
            Self::Settings => keybinds.settings.label(),
            Self::WorkspacePicker => keybinds.workspace_picker.label(),
            Self::NewWorkspace => keybinds.new_workspace.label(),
            Self::NewWorktree => keybinds.new_worktree.label(),
            Self::OpenWorktree => keybinds.open_worktree.label(),
            Self::RemoveWorktree => keybinds.remove_worktree.label(),
            Self::RenameWorkspace => keybinds.rename_workspace.label(),
            Self::CloseWorkspace => keybinds.close_workspace.label(),
            Self::Goto => keybinds.goto.label(),
            Self::Detach => keybinds.detach.label(),
            Self::ReloadConfig => keybinds.reload_config.label(),
            Self::OpenNotificationTarget => keybinds.open_notification_target.label(),
            Self::NewTab => keybinds.new_tab.label(),
            Self::RenameTab => keybinds.rename_tab.label(),
            Self::PreviousTab => keybinds.previous_tab.label(),
            Self::NextTab => keybinds.next_tab.label(),
            Self::CloseTab => keybinds.close_tab.label(),
            Self::RenamePane => keybinds.rename_pane.label(),
            Self::EditScrollback => keybinds.edit_scrollback.label(),
            Self::CopyMode => keybinds.copy_mode.label(),
            Self::FocusPaneLeft => keybinds.focus_pane_left.label(),
            Self::FocusPaneDown => keybinds.focus_pane_down.label(),
            Self::FocusPaneUp => keybinds.focus_pane_up.label(),
            Self::FocusPaneRight => keybinds.focus_pane_right.label(),
            Self::SwapPaneLeft => keybinds.swap_pane_left.label(),
            Self::SwapPaneDown => keybinds.swap_pane_down.label(),
            Self::SwapPaneUp => keybinds.swap_pane_up.label(),
            Self::SwapPaneRight => keybinds.swap_pane_right.label(),
            Self::CyclePaneNext => keybinds.cycle_pane_next.label(),
            Self::CyclePanePrevious => keybinds.cycle_pane_previous.label(),
            Self::LastPane => keybinds.last_pane.label(),
            Self::SplitVertical => keybinds.split_vertical.label(),
            Self::SplitHorizontal => keybinds.split_horizontal.label(),
...
impl ExperimentSetting {
    ...
    pub(crate) fn enabled(self, state: &AppState) -> bool {
        match self {
            ...
            Self::CloudHost => state.cloud_host_enabled(),
        }
    }
}
...
pub struct SettingsState {
    ...
    /// `Some` while the Keybinds tab is waiting for the next keypress to
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum KeybindCaptureKind {
    /// Saved as-is, e.g. `ctrl+alt+right` — fires immediately, no leader key.
    Direct,
    /// Saved as `prefix+<combo>`, e.g. `prefix+n` — goes through the leader key.
    Prefix,
}
...
pub struct ToastNotification {
    ...
    pub title: String,
    pub context: String,
    pub position: Option<crate::config::ToastTanukiPosition>,
...
pub struct AppState {
    ...
    /// Last reported focus state for the outer terminal hosting tanuki.
    ...
    /// Capture mouse input for Tanuki's own mouse UI. When false, Tanuki only
    ...
    /// Configured UI language preference: "auto", "en", or "ru".
    pub language: String,
    ...
    /// Installed or linked plugins known to this running Tanuki instance.
...
impl AppState {
    ...
    pub fn cloud_host_enabled(&self) -> bool {
        self.cloud_host
    }
...
impl AppState {
    ...
    pub fn test_new() -> Self {
        Self {
            ...
            worktree_directory: std::path::PathBuf::from("/tmp/tanuki-worktrees"),
            ...
            update_install_command: "tanuki update".into(),
