...
const DISABLE_SOUND_ENV: &str = "TANUKI_DISABLE_SOUND";
#[cfg(any(windows, test))]
const WINDOWS_SOUND_PATH_ENV: &str = "TANUKI_SOUND_PATH";
#[cfg(not(any(windows, target_os = "macos")))]
...
fn temp_sound_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("tanuki-sound-{}-{id}.mp3", std::process::id()))
}
...
$Path = [Environment]::GetEnvironmentVariable('TANUKI_SOUND_PATH', 'Process')
if ([string]::IsNullOrWhiteSpace($Path)) { throw 'TANUKI_SOUND_PATH is not set' }
...
mod tests {
    ...
    fn windows_media_player_uses_process_environment_and_dispatcher() {
        ...
        assert!(script.contains("GetEnvironmentVariable('TANUKI_SOUND_PATH', 'Process')"));
