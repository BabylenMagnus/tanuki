...
pub const SESSION_ENV_VAR: &str = "TANUKI_SESSION";
...
pub fn configure_from_args(args: &[String]) -> Result<Vec<String>, String> {
        ...
        && args.get(2).map(String::as_str) == Some("attach")
    {
        ...
        let Some(name) = args.get(3) else {
            return Err("usage: tanuki session attach <name>".to_string());
        };
        if args.len() != 4 {
            return Err("usage: tanuki session attach <name>".to_string());
        }
...
pub fn local_attach_command() -> String {
    match active_name() {
        Some(name) => format!("tanuki session attach {name}"),
        None => "tanuki".to_string(),
    }
}
...
pub fn stop_command_for(name: Option<&str>) -> String {
    match name {
        Some(name) => format!("tanuki session stop {name}"),
        None => "tanuki server stop".to_string(),
    }
}
...
pub fn restart_after_update_guidance(stop_command: &str, attach_command: Option<&str>) -> String {
    let restart = match attach_command {
        ...
        None => format!("Run `{stop_command}`, then restart Tanuki with the same socket override."),
    };
    format!(
...
pub fn api_socket_path_for(name: Option<&str>) -> PathBuf {
    data_dir_for(name).join("tanuki.sock")
}
...
pub fn client_socket_path_for(name: Option<&str>) -> PathBuf {
    data_dir_for(name).join("tanuki-client.sock")
}
...
mod tests {
    ...
    fn unique_test_path(name: &str) -> std::path::PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        // See the matching helper in api/server.rs: sockaddr_un's sun_path is
        // capped (~104-108 bytes) and macOS's std::env::temp_dir() can be long
        // enough on its own to overflow it once a name+pid+nanos suffix is
        // appended, so bind under the short, stable `/tmp` instead.
        std::path::PathBuf::from("/tmp").join(format!("h{}-{nanos}.sock", std::process::id()))
    }
    ...
    fn configure_from_args_removes_global_session_option() {
        ...
        assert_eq!(cleaned, vec!["tanuki", "workspace", "list"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn configure_from_args_accepts_equals_form() {
        ...
        assert!(explicit_session_requested());
        assert_eq!(cleaned, vec!["tanuki", "server", "stop"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn configure_from_args_maps_default_session_name_to_default_path() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("tanuki-session-default-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
    ...
    fn env_default_session_name_uses_default_path() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("tanuki-env-session-default-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
    ...
    fn local_attach_command_uses_default_launch_for_default_session() {
        ...
        assert_eq!(local_attach_command(), "tanuki");
    }
    ...
    fn local_attach_command_uses_session_attach_for_named_session() {
        ...
        assert_eq!(local_attach_command(), "tanuki session attach work");
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "tanuki server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "tanuki session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            restart_after_update_guidance(
                "tanuki session stop work",
                Some("tanuki session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `tanuki session stop work`, then run `tanuki session attach work` again."
        );
    }
    ...
    fn active_restart_after_update_guidance_respects_socket_override() {
        ...
        assert_eq!(
            ...
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `TANUKI_SOCKET_PATH=/tmp/custom-tanuki.sock tanuki server stop`, then restart Tanuki with the same socket override."
        );
    ...
    fn explicit_session_socket_ignores_inherited_socket_override() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("tanuki-session-precedence-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
        std::env::set_var(SESSION_ENV_VAR, "work");
        EXPLICIT_SESSION_REQUESTED.store(true, Ordering::Relaxed);
        std::env::set_var(crate::api::SOCKET_PATH_ENV_VAR, "/tmp/inherited.sock");
    ...
    fn env_socket_override_skips_invalid_env_session_validation_without_explicit_session() {
        ...
        std::env::set_var(SESSION_ENV_VAR, "bad/name");
        clear_explicit_session_for_test();
        std::env::set_var(crate::api::SOCKET_PATH_ENV_VAR, "/tmp/tanuki.sock");
        let args = vec![
            "tanuki".to_string(),
            "workspace".to_string(),
            "list".to_string(),
        ...
        assert_eq!(cleaned, vec!["tanuki", "workspace", "list"]);
        assert!(!explicit_session_requested());
        assert_eq!(active_api_socket_path(), PathBuf::from("/tmp/tanuki.sock"));
    ...
    fn list_sessions_skips_reserved_default_directory() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("tanuki-session-list-{}", std::process::id()));
