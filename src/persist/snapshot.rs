...
/// Serializable snapshot of the entire tanuki session.
...
mod tests {
    ...
    fn session_fixture(name: &str) -> &'static str {
        match name {
            "current-tanuki" => {
                include_str!("../../tests/fixtures/session/current-tanuki-session.json")
            }
            "current-tanuki-dev" => {
                include_str!("../../tests/fixtures/session/current-tanuki-dev-session.json")
            }
            "legacy-pre-tabs-v2" => {
    ...
    fn round_trip_full_workspace_snapshot() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/home/can/Projects/tanuki"),
                label: None,
                agent_name: None,
        ...
        let snap = SessionSnapshot {
            workspaces: vec![WorkspaceSnapshot {
                ...
                identity_cwd: PathBuf::from("/home/can/Projects/tanuki"),
                worktree_space: None,
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/can/Projects/tanuki")
        );
        assert_eq!(
    ...
    fn current_session_fixture_parses() {
        let snap = parse_snapshot(session_fixture("current-tanuki")).unwrap();
    ...
    fn current_dev_session_fixture_parses_additive_fields() {
        let snap = parse_snapshot(session_fixture("current-tanuki-dev")).unwrap();
    ...
    fn legacy_workspace_snapshot_migrates_to_single_tab() {
        ...
        assert_eq!(ws.tabs[0].panes[&1].cwd, PathBuf::from("/tmp/tanuki"));
    }
    ...
    fn capture_contract_tracks_worktree_space_membership() {
        ...
        state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "tanuki".into(),
            repo_root: PathBuf::from("/repo/tanuki"),
            checkout_path: PathBuf::from("/repo/tanuki/worktree-a"),
    ...
    fn capture_contract_tracks_workspace_identity_and_pane_cwds() {
        ...
        let second_terminal_id = state.workspaces[0].tabs[0].panes[&second]
            .attached_terminal_id
            .clone();
        state.terminals.get_mut(&second_terminal_id).unwrap().cwd = PathBuf::from("/tmp/tanuki");
        ...
        assert_eq!(tab.panes[&second.raw()].cwd, PathBuf::from("/tmp/tanuki"));
    }
    ...
    fn capture_contract_tracks_hook_authority_agent_session() {
        ...
        assert_eq!(agent_session.source, "tanuki:pi");
    ...
    fn capture_contract_preserves_restored_agent_session() {
        ...
        assert_eq!(agent_session.source, "tanuki:opencode");
    ...
    fn restore_falls_back_to_home_when_cwd_missing() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/tmp/this-directory-does-not-exist-for-tanuki-test"),
                label: None,
                agent_name: None,
                managed_agent_kind: None,
                agent_session: None,
        ...
        assert_eq!(
            restored.workspaces[0].tabs[0].panes[&0].cwd,
            PathBuf::from("/tmp/this-directory-does-not-exist-for-tanuki-test")
        );
    }
}
