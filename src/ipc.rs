...
/// Abstraction over the underlying byte-stream transport used to talk to a
/// thin client or headless server.
///
/// [`Transport::Local`] wraps the existing `interprocess` local socket /
/// named pipe stream. [`Transport::Cloud`] wraps a synthesized duplex byte
/// stream relayed through the Tanuki backend over Socket.IO (see
/// `crate::remote::cloud`), used for the `--cloud-host` / `--attach --cloud`
/// modes instead of a real local socket. Call sites depend on this
/// abstraction rather than `LocalStream` directly.
pub(crate) enum Transport {
    Local(LocalStream),
    Cloud(crate::remote::cloud::CloudDuplex),
}
...
impl Read for Transport {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match self {
            ...
            Transport::Cloud(duplex) => duplex.read(buf),
        }
    }
}
...
impl Write for Transport {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        match self {
            ...
            Transport::Cloud(duplex) => duplex.write(buf),
        }
    }

    fn flush(&mut self) -> io::Result<()> {
        match self {
            Transport::Local(stream) => stream.flush(),
            Transport::Cloud(duplex) => duplex.flush(),
        }
    }
}
...
impl Transport {
    pub(crate) fn try_clone(&self) -> io::Result<Transport> {
        match self {
            Transport::Local(stream) => Ok(Transport::Local(stream.try_clone()?)),
            Transport::Cloud(duplex) => Ok(Transport::Cloud(duplex.clone())),
        }
    }

    pub(crate) fn set_nonblocking(&self, enabled: bool) -> io::Result<()> {
        match self {
            Transport::Local(stream) => stream.set_nonblocking(enabled),
            // The cloud transport has no underlying OS socket to toggle;
            // blocking behavior is implemented at the internal channel
            // level in `CloudDuplex::read` instead. No-op, matching the
            // existing precedent of `Unsupported`-tolerant paths elsewhere
            // in this abstraction (e.g. Windows recv-timeout handling in
            // `server/client_transport.rs`).
...
mod tests {
    ...
    fn temp_socket_marker_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("tanuki-{name}-{}.sock", std::process::id()))
    }
}
