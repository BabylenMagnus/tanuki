...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: tanuki terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: tanuki terminal session control <target> [--takeover] [--cols N] [--rows N]";
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: tanuki channel set <stable|preview>");
        return Ok(2);
    };
    ...
    println!(
        "Tanuki update channel set to {channel} in {}.",
        path.display()
    );
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `tanuki update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("tanuki channel commands:");
    eprintln!("  tanuki channel show                  print the configured update channel");
    eprintln!("  tanuki channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: tanuki config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: tanuki config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: tanuki config reset-keys");
        return Ok(2);
    }
    ...
    println!("Built-in v2 keybindings will apply after Tanuki restarts or reloads config.");
    println!("If a Tanuki server is running, run `tanuki server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: tanuki session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: tanuki session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: tanuki session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: tanuki session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: tanuki session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: tanuki terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: tanuki terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: tanuki terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: tanuki terminal title set <title>");
            eprintln!("       tanuki terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: tanuki terminal title set <title>");
...
fn print_config_help() {
    eprintln!("tanuki config commands:");
    eprintln!("  tanuki config check  validate config.toml and print diagnostics");
    eprintln!("  tanuki config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("tanuki terminal commands:");
    eprintln!("  tanuki terminal attach <terminal_id> [--takeover]");
    eprintln!("  tanuki terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  tanuki terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  tanuki terminal title set <title>");
    eprintln!("  tanuki terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_session_help() {
    eprintln!("tanuki session commands:");
    eprintln!("  tanuki session list [--json]");
    eprintln!("  tanuki session attach <name>");
    eprintln!("  tanuki session stop <name> [--json]");
    eprintln!("  tanuki session delete <name> [--json]");
...
mod tests {
    ...
    fn parse_env_assignment_accepts_empty_values() {
        assert_eq!(
            super::parse_env_assignment("TANUKI_ROLE=").unwrap(),
            ("TANUKI_ROLE".to_string(), String::new())
        );
    }
    ...
    fn parse_env_assignment_requires_key_value_separator() {
        assert_eq!(
            super::parse_env_assignment("TANUKI_ROLE").unwrap_err(),
