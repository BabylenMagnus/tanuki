...
fn default_update_channel() -> UpdateChannelConfig {
    // Unlike upstream Herdr, Tanuki ships real stable Windows builds
    // (see release.yml) — Windows does not need to default to preview.
...
pub struct ToastConfig {
    ...
    pub tanuki: TanukiToastConfig,
    pub clipboard: ClipboardToastConfig,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
pub struct TanukiToastConfig {
    pub position: ToastTanukiPosition,
}
...
pub struct SessionConfig {
    ...
    /// when restoring a Tanuki session. Default: true.
...
pub struct KeysConfig {
    ...
    /// Resize every split in the current tab to be equal-sized. Default: "prefix+="
...
pub struct WorktreesConfig {
    /// Root directory under which Tanuki creates <repo>/<branch-slug> checkouts.
...
pub struct UiConfig {
    ...
    /// Terminal width at or below which Tanuki uses the mobile single-column layout. Default: 64.
    pub mobile_width_threshold: u16,
    /// Capture mouse input for Tanuki's mouse UI. Default: true.
...
pub struct ExperimentalConfig {
    /// Allow launching tanuki inside an existing tanuki pane. Default: false.
...
impl Default for KeysConfig {
    fn default() -> Self {
        Self {
            ...
            equalize_splits: BindingConfig::one("prefix+="),
...
impl Default for WorktreesConfig {
    fn default() -> Self {
        Self {
            directory: "~/.tanuki/worktrees".into(),
        }
    }
}
...
impl Default for ToastConfig {
    fn default() -> Self {
        Self {
            ...
            delay_seconds: 1,
            tanuki: TanukiToastConfig::default(),
            clipboard: ClipboardToastConfig::default(),
        }
    }
}

impl Default for TanukiToastConfig {
    fn default() -> Self {
        Self {
            position: ToastTanukiPosition::BottomRight,
        }
    }
}
...
"#;
    ...
    fn worktrees_directory_defaults_and_parses() {
        let default_config = Config::default();
        assert_eq!(default_config.worktrees.directory, "~/.tanuki/worktrees");

        let toml = r#"
[worktrees]
directory = "~/Projects/tanuki-worktrees"
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.worktrees.directory, "~/Projects/tanuki-worktrees");
    }
...
"#;
    ...
    fn toast_config_defaults_preserve_existing_behavior_with_delay() {
        ...
        assert_eq!(
            config.ui.toast.tanuki.position,
            ToastTanukiPosition::BottomRight
        );
        assert!(config.ui.toast.clipboard.enabled);
        assert_eq!(
            config.ui.toast.clipboard.position,
...
"#;
    ...
    #[test]
    fn toast_config_legacy_enabled_true_maps_to_tanuki() {
        let toml = r#"
[ui.toast]
enabled = true
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.ui.toast.delivery, ToastDelivery::Tanuki);
    }
